...
async function load() {
    try {
        ...
        if(histRes.ok) {
            ...
            let fbRes = await fetch('/api/history?t=' + ts);
            let fbData = await fbRes.json();
            globalHistory = Array.isArray(fbData) ? fbData : [];
        }
...
// 💡 📊 [주간 보고서 UI 100% 복구본 - 가용률, 3개월 로직 모두 포함]
