    ...
    container.innerHTML = `
        <div class="w-full max-w-7xl mx-auto space-y-6">
            <div class="flex items-center justify-between mb-2">
                <h2 class="text-2xl font-black text-slate-800">🚚 출고 및 서류 관리</h2>
            </div>
            
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div class="flex-1">
                        <label class="block text-xs font-black text-slate-500 mb-2">1. 고객사(발주서 양식) 선택</label>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                <div class="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                    <div class="font-black text-slate-700">📋 정제된 데이터 미리보기 <span id="outbound-count" class="text-indigo-600 ml-2">0건</span></div>
                ...
                <div class="overflow-x-auto max-h-[600px] overflow-y-auto">
                    <table class="w-full text-left min-w-max border-collapse">
                        ...
                        <tbody id="outbound-tbody" class="divide-y divide-slate-100">
                            <tr><td colspan="6" class="p-10 text-center text-slate-400 font-bold">발주서를 업로드하시면 표준 데이터로 자동 변환됩니다.</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        try {
            ...
            for(let i = mapping.headerRow + 1; i < rawData.length; i++) {
                ...
                parsedOutboundData.push({
                    ...
                    unit_price: parseQuantity(rawPrice, mapping.parsePrice)
                });
            }
...
function renderOutboundPreview() {
    ...
    tbody.innerHTML = parsedOutboundData.map((item, idx) => `
        <tr class="hover:bg-indigo-50/50 transition-colors border-b border-slate-100">
            ...
            <td class="p-3 text-xs font-bold text-slate-600">${item.expected_date || '<span class="text-orange-500">없음</span>'}</td>
...
function confirmOutboundData() {
    alert("출고 2단계 (품목 매핑 및 서류 생성) 로직으로 넘어갈 준비가 되었습니다!");
}
