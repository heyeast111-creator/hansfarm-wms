...
<body class="font-sans h-screen flex overflow-hidden text-slate-800 selection:bg-indigo-200">
    ...
    <script>
        ...
        async function clickCell(displayId, searchId) {
            ...
            panelHtml += `
                <div class="mt-4 pt-4 border-t border-slate-200">
                    ...
                    <div class="space-y-2">
                        <div class="flex space-x-2"><div class="w-1/3"><label class="block text-[10px] font-bold text-slate-400 mb-1">카테고리</label><select id="in-cat" onchange="updateProductDropdown()" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"><option value="">선택</option>${catOptions}</select></div><div class="w-2/3"><label class="block text-[10px] font-bold text-slate-400 mb-1">품목명</label><select id="in-item" onchange="updateSupplierDropdown()" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"><option value="">카테고리 선택</option></select></div></div>
        ...
        function updateProductDropdown() { 
            ...
            const items = [...new Set(productMaster.filter(p => p.category === cat).map(p => p.item_name))];
            document.getElementById('in-item').innerHTML = items.map(name => `<option value="${name}">${name}</option>`).join(''); 
            updateSupplierDropdown();
        }
        
        function updateSupplierDropdown() {
            const cat = document.getElementById('in-cat').value; const item = document.getElementById('in-item').value;
            const suppliers = productMaster.filter(p => p.category === cat && p.item_name === item).map(p => p.supplier);
        ...
        function exportExcel() {
            ...
            if(globalOccupancy.length === 0) {
                ...
                wsData = globalOccupancy.map(item => {
                    let pInfo = productMaster.find(p => p.item_name === item.item_name && p.supplier === item.remarks); 
