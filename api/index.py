...
<body class="font-sans h-screen flex flex-col md:flex-row overflow-hidden text-slate-800 selection:bg-indigo-200 pb-16 md:pb-0">
    ...
    <aside class="w-full md:w-36 bg-white border-t md:border-t-0 md:border-r border-slate-300 flex flex-row md:flex-col items-center py-1 md:py-6 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] md:shadow-lg z-50 shrink-0 fixed md:relative bottom-0 left-0 h-16 md:h-screen">
        <div class="hidden md:flex mb-8 w-full px-4 justify-center"><img src="/logo.jpg" alt="HANS FARM" class="max-w-full h-auto object-contain drop-shadow-sm"></div>
        <div class="flex flex-row md:flex-col md:space-y-2 w-full px-1 md:px-3 flex-1 justify-around md:justify-start">
            <button onclick="showView('dashboard')" id="nav-dashboard" class="nav-btn inactive flex-1 md:w-full py-2 md:py-3 rounded-md text-[11px] md:text-sm border flex flex-col md:flex-row items-center justify-center mx-0.5 md:mx-0 md:mb-2">📊<span class="mt-1 md:mt-0 md:ml-1">대시보드</span></button>
            <button onclick="showView('inventory')" id="nav-inventory" class="nav-btn inactive flex-1 md:w-full py-2 md:py-3 rounded-md text-[11px] md:text-sm border flex flex-col md:flex-row items-center justify-center mx-0.5 md:mx-0 md:mb-2">🗺️<span class="mt-1 md:mt-0 md:ml-1">렉맵</span></button>
            <button onclick="showView('search')" id="nav-search" class="nav-btn inactive flex-1 md:w-full py-2 md:py-3 rounded-md text-[11px] md:text-sm border flex flex-col md:flex-row items-center justify-center mx-0.5 md:mx-0 md:mb-2">📦<span class="mt-1 md:mt-0 md:ml-1">재고조회</span></button>
            <button onclick="showView('production')" id="nav-production" class="nav-btn inactive hidden md:flex flex-1 md:w-full py-2 md:py-3 rounded-md text-[11px] md:text-sm border items-center justify-center flex-col md:flex-row md:mb-2 mx-0.5 md:mx-0">생산관리</button>
            <button onclick="showView('outbound')" id="nav-outbound" class="nav-btn inactive hidden md:flex flex-1 md:w-full py-2 md:py-3 rounded-md text-[11px] md:text-sm border items-center justify-center flex-col md:flex-row md:mb-2 mx-0.5 md:mx-0">출고관리</button>
            <button onclick="showView('safety')" id="nav-safety" class="nav-btn inactive hidden md:flex flex-1 md:w-full py-2 md:py-3 rounded-md text-[11px] md:text-sm border items-center justify-center flex-col md:flex-row md:mb-2 mx-0.5 md:mx-0">안전재고</button>
            <button onclick="showView('products')" id="nav-products" class="nav-btn inactive hidden md:flex flex-1 md:w-full py-2 md:py-3 rounded-md text-[11px] md:text-sm border items-center justify-center flex-col md:flex-row md:mb-2 mx-0.5 md:mx-0">품목관리</button>
            <button onclick="showView('accounting')" id="nav-accounting" class="nav-btn inactive hidden flex-1 md:w-full py-2 md:py-3 rounded-md text-[11px] md:text-sm border items-center justify-center flex-col md:flex-row md:mb-2 mx-0.5 md:mx-0">정산회계</button>
            <div class="hidden md:block my-2 border-b border-slate-200 w-full"></div>
            <button onclick="exportPhysicalCountExcel()" class="hidden md:block w-full py-3 rounded-md bg-blue-50 border border-blue-200 text-blue-700 font-black shadow-inner text-[12px] transition-all hover:bg-blue-100">📋 실사 양식</button>
            <button onclick="document.getElementById('excel-upload').click()" class="hidden md:block w-full mt-1 py-3 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 font-black shadow-inner text-[12px] transition-all hover:bg-emerald-100">⬆️ 엑셀 입고</button>
        </div>
        <div class="hidden md:block w-full px-3 mt-auto pb-4">
            <button id="admin-btn" onclick="adminLogin()" class="w-full py-3 rounded-md bg-slate-800 border border-slate-900 text-slate-300 font-black shadow-inner text-[11px] transition-all hover:bg-slate-700 hover:text-white">🔒 관리자</button>
        </div>
    </aside>
    ...
    <main class="flex-1 bg-slate-200 relative overflow-hidden flex flex-col h-full w-full">
        ...
        <div id="view-dashboard" class="flex flex-col h-full w-full absolute inset-0 p-4 md:p-8 overflow-auto z-10 bg-slate-100">
            <div class="flex flex-col md:flex-row justify-between items-center mb-6 space-y-2 md:space-y-0"><h1 class="text-2xl md:text-3xl font-black text-slate-800">📊 물류 대시보드</h1><select id="dash-period" onchange="updateDashboard()" class="w-full md:w-auto border-2 border-indigo-200 rounded-lg p-2 font-bold text-indigo-700 bg-white outline-none"><option value="daily">일간 (Daily)</option><option value="weekly">주간 (Weekly)</option><option value="monthly">월간 (Monthly)</option></select></div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div class="col-span-1 md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="bg-indigo-50 p-5 rounded-2xl shadow-sm border border-indigo-100 flex flex-col justify-between"><div class="flex justify-between items-center mb-2"><div class="font-black text-indigo-800 text-sm md:text-base">❄️ 냉장 창고 (Cold)</div><div class="text-indigo-500 font-bold text-sm" id="dash-cold-percent">0%</div></div><div class="space-y-2 text-xs md:text-sm mt-2"><div class="flex justify-between text-slate-600"><span class="font-bold">총 렉 수용량</span><span class="font-black" id="dash-cold-total">0</span></div><div class="flex justify-between text-indigo-600"><span class="font-bold">적재 볼륨</span><span class="font-black" id="dash-cold-occ">0</span></div><div class="flex justify-between text-emerald-600"><span class="font-bold">빈 렉 수</span><span class="font-black" id="dash-cold-empty">0</span></div></div></div>
                    <div class="bg-orange-50 p-5 rounded-2xl shadow-sm border border-orange-100 flex flex-col justify-between"><div class="flex justify-between items-center mb-2"><div class="font-black text-orange-800 text-sm md:text-base">☀️ 실온 창고 (Room)</div><div class="text-orange-500 font-bold text-sm" id="dash-room-percent">0%</div></div><div class="space-y-2 text-xs md:text-sm mt-2"><div class="flex justify-between text-slate-600"><span class="font-bold">총 렉 수용량</span><span class="font-black" id="dash-room-total">0</span></div><div class="flex justify-between text-orange-600"><span class="font-bold">적재 볼륨</span><span class="font-black" id="dash-room-occ">0</span></div><div class="flex justify-between text-emerald-600"><span class="font-bold">빈 렉 수</span><span class="font-black" id="dash-room-empty">0</span></div></div></div>
                </div>
            </div>
        ...
        <div id="view-search" class="hidden flex-col items-center justify-start md:justify-center h-full w-full absolute inset-0 p-4 md:p-8 overflow-auto z-10 bg-slate-100">
            ...
            <div class="bg-white p-4 md:p-8 rounded-2xl shadow-xl border border-slate-200 w-full max-w-5xl text-center">
                <div class="flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-3 mb-6">
                    <div class="w-full md:w-1/4 text-left"><label class="block text-[11px] font-bold text-slate-500 mb-1">① 관리 유형</label><select id="summary-type" onchange="updateSummarySupplierDropdown()" class="w-full text-sm p-3 border-2 border-slate-300 rounded-xl font-bold text-slate-800 outline-none bg-slate-50"><option value="ALL">전체 유형</option><option value="FINISHED">제품 (완제품)</option><option value="MATERIAL">자재 (부자재)</option><option value="RAW">원란 (냉장)</option></select></div>
                    <div class="w-full md:w-1/4 text-left"><label class="block text-[11px] font-bold text-slate-500 mb-1">② 입고처 (생산처)</label><select id="summary-supplier" onchange="updateSummaryCategoryDropdown()" class="w-full text-sm p-3 border-2 border-rose-200 rounded-xl font-bold text-rose-800 outline-none bg-white"><option value="ALL">전체 입고처</option></select></div>
                    <div class="w-full md:w-1/4 text-left"><label class="block text-[11px] font-bold text-slate-500 mb-1">③ 카테고리</label><select id="summary-category" onchange="updateSummaryItemDropdown()" class="w-full text-sm p-3 border-2 border-indigo-200 rounded-xl font-bold text-indigo-800 outline-none bg-white"><option value="ALL">전체 카테고리</option></select></div>
                    <div class="w-full md:w-1/4 text-left"><label class="block text-[11px] font-bold text-slate-500 mb-1">④ 품목명 확인</label><select id="summary-item" onchange="calculateSummary()" class="w-full text-sm p-3 border-2 border-emerald-200 rounded-xl font-bold text-emerald-800 outline-none bg-white"><option value="">앞에서부터 선택해주세요</option></select></div>
                </div>
                <div class="mt-4 md:mt-8 p-6 bg-slate-50 border border-slate-200 rounded-xl shadow-inner">
                    ...
                    <div class="text-xs md:text-sm font-bold text-rose-500 mt-2 md:mt-3 mb-4 md:mb-6" id="summary-pallet">0.0 P (적재 부피 합산)</div>
                    <button onclick="findItemLocationFromSummary()" class="bg-indigo-600 hover:bg-indigo-700 text-white font-black py-3 px-4 md:px-8 rounded-xl shadow-md transition-colors w-full max-w-sm mx-auto flex items-center justify-center text-sm md:text-base"><svg class="w-4 h-4 md:w-5 md:h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>📍 렉맵에서 위치 바로 확인</button>
                </div>
            </div>
        </div>

        <div id="view-safety" class="hidden flex-col h-full w-full absolute inset-0 p-8 overflow-auto z-10 bg-slate-100">
            <h1 class="text-3xl font-black text-slate-800 mb-8">발주 및 안전재고 (PC 전용 화면입니다)</h1>
        ...
        <div id="view-products" class="hidden flex-col h-full w-full absolute inset-0 p-8 overflow-auto z-10 bg-slate-100">
            <h1 class="text-3xl font-black text-slate-800 mb-8">마스터 DB (PC 전용 화면입니다)</h1>
            <div id="fp-list"></div><div id="pm-list"></div><div id="bom-list"></div>
        </div>
        <div id="view-accounting" class="hidden flex-col h-full w-full absolute inset-0 p-8 overflow-auto z-10 bg-slate-100">
            <h1 class="text-3xl font-black text-slate-800 mb-8">정산 및 회계 (PC 전용 화면입니다)</h1>
            <div id="acc-list"></div>
        </div>
    ...
    <script>
        ...
        function showView(viewName) {
            document.querySelectorAll('.nav-btn').forEach(btn => { btn.classList.remove('active', 'active-safety', 'active-accounting'); btn.classList.add('inactive'); });
            let targetBtn = document.getElementById('nav-' + viewName);
            if(targetBtn) { targetBtn.classList.remove('inactive'); if(viewName === 'safety') targetBtn.classList.add('active-safety'); else if(viewName === 'accounting') targetBtn.classList.add('active-accounting'); else targetBtn.classList.add('active'); }
        ...
        function calculateSummary() {
            ...
            let totalQty = 0, totalPallet = 0;
            if(itemName) { globalOccupancy.forEach(item => { let itemSupplier = item.remarks || "기본입고처"; if(item.item_name === itemName) { if (supplier === 'ALL' || itemSupplier === supplier) { totalQty += item.quantity; totalPallet += getDynamicPalletCount(item); } } }); }
        ...
        async function clickCell(displayId, searchId) { 
            ...
            if(items.length > 0) { 
                ...
                items.forEach(item => { 
                    ...
                    let editBtn = `<button onclick="editInventoryItem('${item.id}', '${item.item_name}', ${item.quantity}, '${item.production_date || ''}', '${searchId}', '${item.remarks || ''}')" class="flex-1 bg-slate-50 hover:bg-slate-200 text-slate-600 border border-slate-200 py-1.5 rounded text-[11px] font-bold transition-colors mt-2 w-full">⚙️ 편집/삭제</button>`;
