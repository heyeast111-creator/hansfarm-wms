...
class ProductData(BaseModel):
    category: str; item_name: str; supplier: str = "기본입고처"; daily_usage: int = 0; unit_price: int = 0
...
<body class="font-sans h-screen flex overflow-hidden text-slate-800 selection:bg-indigo-200">
    ...
    <aside class="w-36 bg-white border-r border-slate-300 flex flex-col items-center py-6 shadow-lg z-20 shrink-0">
        ...
        <div class="flex flex-col space-y-2 w-full px-3 flex-1">
            ...
            <button onclick="exportPhysicalCountExcel()" class="w-full py-3 rounded-md bg-blue-50 border border-blue-200 text-blue-700 font-black shadow-inner text-[12px] transition-all hover:bg-blue-100">📋 실사 양식 다운</button>
            <div class="flex space-x-1">
                <button onclick="document.getElementById('excel-upload').click()" class="flex-1 py-2 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 font-black shadow-inner text-[10px] transition-all hover:bg-emerald-100">가져오기</button>
                <button onclick="exportExcel()" class="flex-1 py-2 rounded-md bg-orange-50 border border-orange-200 text-orange-700 font-black shadow-inner text-[10px] transition-all hover:bg-orange-100">내보내기</button>
            </div>
        </div>
    ...
    <main class="flex-1 bg-slate-200 relative overflow-hidden flex flex-col">
        ...
        <div id="view-inventory" class="hidden flex-col h-full w-full absolute inset-0 transition-opacity duration-300">
            <header class="h-16 bg-white border-b border-slate-300 flex items-end justify-between px-6 pt-4 shrink-0 shadow-sm z-10">
                <div class="flex space-x-1">
                    <button id="tab-room" onclick="switchZone('실온')" class="px-8 py-2.5 bg-orange-500 text-white font-bold rounded-t-lg shadow-inner">실온</button>
                    <button id="tab-cold" onclick="switchZone('냉장')" class="px-8 py-2.5 bg-slate-100 border-x border-t border-slate-300 text-slate-500 font-bold rounded-t-lg hover:bg-slate-200">냉장</button>
                    <button id="tab-floor" onclick="switchZone('현장')" class="px-8 py-2.5 bg-slate-100 border-x border-t border-slate-300 text-slate-500 font-bold rounded-t-lg hover:bg-slate-200">생산 현장</button>
                </div>
        ...
        <div id="view-dashboard" class="flex flex-col h-full w-full absolute inset-0 p-8 overflow-auto z-10 bg-slate-100">
            ...
            <div id="admin-finance-panel" class="hidden grid grid-cols-2 gap-6 mb-6">
                <div class="bg-yellow-50 p-6 rounded-2xl shadow-md border border-yellow-200">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-center text-sm font-bold text-yellow-700 border-b border-yellow-200 pb-2"><span>❄️ 냉장</span><span id="dash-val-cold">0 원</span></div>
                        <div class="flex justify-between items-center text-sm font-bold text-yellow-700 border-b border-yellow-200 pb-2"><span>☀️ 실온</span><span id="dash-val-room">0 원</span></div>
        ...
        <div id="view-search" class="hidden flex-col items-center justify-center h-full w-full absolute inset-0 p-8 z-10 bg-slate-100">
            <h1 class="text-3xl font-black text-slate-800 mb-8">🔍 검색</h1>
            <div class="bg-white p-8 rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl text-center">
                ...
                <button onclick="executeSearch()" class="bg-indigo-600 hover:bg-indigo-700 text-white font-black text-lg py-4 px-12 rounded-xl shadow-lg transition-all w-full">렉 찾기기</button>
            </div>
        </div>
        ...
        <div id="view-safety" class="hidden flex-col h-full w-full absolute inset-0 p-8 overflow-auto z-10 bg-slate-100">
            ...
            <div class="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
                <table class="w-full text-left border-collapse">
                    <thead><tr class="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm"><th class="p-4 font-black">카테고리</th><th class="p-4 font-black">품목명 (입고처)</th><th class="p-4 font-black text-right">현재 총 재고</th><th class="p-4 font-black text-right">일간 소모량</th><th class="p-4 font-black text-center">예상 소진일</th><th class="p-4 font-black text-center">상태 및 발주</th></tr></thead>
        ...
        <div id="view-products" class="hidden flex-col h-full w-full absolute inset-0 p-8 overflow-auto z-10 bg-slate-100">
            <div class="flex justify-between items-center mb-8 border-b pb-4">
                ...
                <div class="flex space-x-2">
                    <button onclick="document.getElementById('product-upload').click()" class="bg-emerald-50 text-emerald-700 font-bold border border-emerald-200 py-2 px-4 rounded-lg shadow-sm hover:bg-emerald-100 transition-colors text-sm">대량 업로드</button>
            ...
            <div class="grid grid-cols-3 gap-8">
                <div class="col-span-1 bg-white p-6 rounded-2xl shadow-md border border-slate-200 h-fit">
                    ...
                    <label class="block text-xs font-bold text-rose-500 mb-1">입고처 (거래처명)</label><input type="text" id="pm-supplier" placeholder="거래처 명 매칭 필수" class="w-full border border-rose-300 bg-rose-50 rounded p-2 mb-3 font-bold text-sm">
        ...
        <div id="view-accounting" class="hidden flex-col h-full w-full absolute inset-0 p-8 overflow-auto z-10 bg-slate-100">
            ...
            <div class="flex space-x-4 mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
                <div>
                    <label class="block text-xs font-bold text-slate-500 mb-1">조회 월 (YYYY-MM)</label>
                    <input type="month" id="acc-month" onchange="renderAccounting()" class="border border-slate-300 rounded p-2 text-sm font-bold text-slate-700 outline-none">
                </div>
                <div class="flex-1">
                    <label class="block text-xs font-bold text-slate-500 mb-1">매입처 (입고처) 선택</label>
                    <select id="acc-supplier" onchange="renderAccounting()" class="w-full border border-slate-300 rounded p-2 text-sm font-bold text-slate-700 outline-none">
                        <option value="ALL">전체 매입처 보기</option>
                    </select>
                </div>
            </div>

            <div class="grid grid-cols-3 gap-6 mb-6">
                <div class="bg-white p-6 rounded-2xl shadow-md border-l-4 border-slate-700">
                    <div class="text-slate-500 font-bold text-sm mb-1">해당 월 총 매입금액</div>
                    <div class="text-3xl font-black text-slate-800" id="acc-total">0 원</div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-md border-l-4 border-rose-500">
                    <div class="text-rose-500 font-bold text-sm mb-1">지급 대기 (미지급금)</div>
                    <div class="text-3xl font-black text-rose-600" id="acc-unpaid">0 원</div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-md border-l-4 border-blue-500">
                    <div class="text-blue-500 font-bold text-sm mb-1">결제 완료</div>
                    <div class="text-3xl font-black text-blue-600" id="acc-paid">0 원</div>
                </div>
            </div>

            <div class="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
                <table class="w-full text-left border-collapse">
                    <thead><tr class="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs"><th class="p-3 font-black">입고일시</th><th class="p-3 font-black">매입처</th><th class="p-3 font-black">품목명</th><th class="p-3 font-black text-right">입고수량</th><th class="p-3 font-black text-right">단가</th><th class="p-3 font-black text-right">매입금액</th><th class="p-3 font-black text-center">결제상태</th></tr></thead>
    ...
    <script>
        ...
        function adminLogin() {
            if(isAdmin) { 
                ...
                document.getElementById('admin-btn').innerText = "🔒 관리자 로그인"; document.getElementById('admin-btn').className = "w-full py-3 rounded-md bg-slate-800 border border-slate-900 text-slate-300 font-black shadow-inner text-[11px] transition-all hover:bg-slate-700 hover:text-white"; 
        ...
        // ==========================================
        // 💰 회계 및 정산 엔진 (Admin Only)
        // ==========================================
        function renderAccounting() {
            const selMonth = document.getElementById('acc-month').value; // YYYY-MM
            if(!selMonth) return;
            
            // 드롭다운 세팅
            let suppliers = [...new Set(globalHistory.filter(h => h.action_type === '입고').map(h => h.remarks || '기본입고처'))];
            let supSelect = document.getElementById('acc-supplier');
            let currentSel = supSelect.value;
            supSelect.innerHTML = `<option value="ALL">전체 매입처 보기</option>` + suppliers.map(s => `<option value="${s}">${s}</option>`).join('');
            if(suppliers.includes(currentSel)) supSelect.value = currentSel;

            const selectedSup = supSelect.value;

            let totalAcc = 0, unpaidAcc = 0, paidAcc = 0;
            let html = '';

            // 필터링: 이번달 + 입고 내역 + (선택된 매입처)
            let filtered = globalHistory.filter(h => {
                let hMonth = h.created_at.substring(0, 7);
                let hSup = h.remarks || '기본입고처';
                return h.action_type === '입고' && hMonth === selMonth && (selectedSup === 'ALL' || selectedSup === hSup);
            });

            filtered.sort((a,b) => new Date(b.created_at) - new Date(a.created_at));

            filtered.forEach(h => {
                let pInfo = productMaster.find(p => p.item_name === h.item_name && p.supplier === (h.remarks || '기본입고처'));
                let price = pInfo ? pInfo.unit_price : 0;
                let cost = price * h.quantity;
                totalAcc += cost;
                
                let isPaid = h.payment_status === '결제완료';
                if(isPaid) paidAcc += cost; else unpaidAcc += cost;

                let dateStr = new Date(h.created_at).toLocaleString('ko-KR', {month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit'});
                let btnHtml = isPaid 
                    ? `<button onclick="togglePayment('${h.id}', '미지급')" class="bg-slate-100 text-slate-500 font-bold px-2 py-1 rounded text-[10px] hover:bg-slate-200">취소</button>`
                    : `<button onclick="togglePayment('${h.id}', '결제완료')" class="bg-blue-500 text-white font-bold px-2 py-1 rounded text-[10px] hover:bg-blue-600 shadow-sm">결제완료 처리</button>`;
                
                let statusTag = isPaid ? `<span class="text-blue-600 font-black text-xs mr-2">✅ 완료</span>` : `<span class="text-rose-500 font-black text-xs mr-2">⏳ 미지급</span>`;

                html += `<tr class="hover:bg-slate-50 transition-colors">
                    <td class="p-3 text-slate-500">${dateStr}</td>
                    <td class="p-3 font-bold text-slate-700">${h.remarks || '기본'}</td>
                    <td class="p-3 font-black text-slate-800">${h.item_name}</td>
                    <td class="p-3 text-right font-bold text-indigo-600">${h.quantity.toLocaleString()}</td>
                    <td class="p-3 text-right text-slate-500">${price.toLocaleString()}</td>
                    <td class="p-3 text-right font-black text-slate-800">${cost.toLocaleString()}</td>
                    <td class="p-3 text-center flex items-center justify-center">${statusTag} ${btnHtml}</td>
                </tr>`;
            });

            document.getElementById('acc-list').innerHTML = html || `<tr><td colspan="7" class="p-10 text-center text-slate-400 font-bold">해당 월에 입고(매입) 내역이 없습니다.</td></tr>`;
            document.getElementById('acc-total').innerText = totalAcc.toLocaleString() + ' 원';
            document.getElementById('acc-unpaid').innerText = unpaidAcc.toLocaleString() + ' 원';
            document.getElementById('acc-paid').innerText = paidAcc.toLocaleString() + ' 원';
        }

        async function togglePayment(logId, status) {
            try {
                await fetch(`/api/history/${logId}`, { method: 'PATCH', headers: {'Content-Type':'application/json'}, body: JSON.stringify({payment_status: status}) });
                load();
            } catch(e) { alert("상태 변경 실패!"); }
        }

        // ==========================================
        // 📊 대시보드 업데이트 (재고자산 평가 포함)
        // ==========================================
        function updateDashboard() {
            ...
            if(isAdmin) document.getElementById('dash-cost-out').innerText = productionCost.toLocaleString() + ' 원';

            let totalRoom = 0, occRoom = 0, totalCold = 0, occCold = 0; 
            let valRoom = 0, valCold = 0;
            const occMap = {}; globalOccupancy.forEach(item => { occMap[item.location_id] = true; });
            
            globalOccupancy.forEach(item => {
                let p = item.pallet_count || 1;
                let pInfo = productMaster.find(prod => prod.item_name === item.item_name && prod.supplier === item.remarks);
                let val = pInfo ? pInfo.unit_price * item.quantity : 0;

                if(item.location_id.startsWith('K-') || layoutRoom.some(col => col.id && item.location_id.startsWith(col.id+'-'))) { occRoom += p; valRoom += val; }
                else if(item.location_id.startsWith('I-') || layoutCold.some(col => col.id && item.location_id.startsWith(col.id+'-'))) { occCold += p; valCold += val; }
                else { valRoom += val; } // 현장(Floor) 자산도 일단 합산
            });
            ...
            document.getElementById('dash-cap-percent').innerText = capRate + '%'; document.getElementById('dash-cap-text').innerText = `${occAll.toFixed(1)} / ${totalAll} 파레트`; 
            let color = capRate > 100 ? '#e11d48' : '#10b981';
            document.getElementById('dash-donut').style.background = `conic-gradient(${color} 0% ${Math.min(capRate, 100)}%, #e2e8f0 ${Math.min(capRate, 100)}% 100%)`;

            document.getElementById('dash-room-total').innerText = totalRoom; document.getElementById('dash-room-occ').innerText = occRoom.toFixed(1); document.getElementById('dash-room-empty').innerText = Math.max(0, totalRoom - Math.floor(occRoom)); document.getElementById('dash-room-percent').innerText = totalRoom > 0 ? Math.round((occRoom/totalRoom)*100) + '%' : '0%';
            document.getElementById('dash-cold-total').innerText = totalCold; document.getElementById('dash-cold-occ').innerText = occCold.toFixed(1); document.getElementById('dash-cold-empty').innerText = Math.max(0, totalCold - Math.floor(occCold)); document.getElementById('dash-cold-percent').innerText = totalCold > 0 ? Math.round((occCold/totalCold)*100) + '%' : '0%';
            
            if(isAdmin) {
                document.getElementById('dash-val-room').innerText = valRoom.toLocaleString() + ' 원';
                document.getElementById('dash-val-cold').innerText = valCold.toLocaleString() + ' 원';
                document.getElementById('dash-val-total').innerText = (valRoom + valCold).toLocaleString() + ' 원';
                document.getElementById('admin-dashboard-panel').innerHTML = `<div class="mt-8 p-6 bg-rose-50 border border-rose-200 rounded-2xl"><h3 class="text-rose-800 font-black text-lg mb-4 flex items-center">🚨 시스템 관리자 전용 구역</h3><div class="flex space-x-4"><button onclick="clearData('inventory')" class="bg-white border border-rose-300 text-rose-600 font-bold py-3 px-6 rounded-lg shadow-sm hover:bg-rose-100 transition-colors">📦 창고 재고 전체 삭제</button><button onclick="clearData('history')" class="bg-white border border-rose-300 text-rose-600 font-bold py-3 px-6 rounded-lg shadow-sm hover:bg-rose-100 transition-colors">📝 히스토리 내역 전체 삭제</button></div></div>`;
            } else { document.getElementById('admin-dashboard-panel').innerHTML = ''; }
        }

        // ==========================================
        // 이하 렉맵 및 입출고 통신 엔진 (유지)
        // ==========================================
        function switchZone(zone) { currentZone = zone; selectedCellId = null; clearInfo(); ['tab-room', 'tab-cold', 'tab-floor'].forEach(id => { document.getElementById(id).className = "px-8 py-2.5 bg-slate-100 border-x border-t border-slate-300 text-slate-500 font-bold rounded-t-lg hover:bg-slate-200"; }); if(zone === '실온') { document.getElementById('tab-room').className = "px-8 py-2.5 bg-orange-500 text-white font-bold rounded-t-lg shadow-inner"; document.getElementById('fifo-btn-container').classList.add('hidden'); document.getElementById('floor-select').classList.remove('hidden'); document.getElementById('floor-select-label').classList.remove('hidden'); } else if(zone === '냉장') { document.getElementById('tab-cold').className = "px-8 py-2.5 bg-indigo-600 text-white font-bold rounded-t-lg shadow-inner"; document.getElementById('fifo-btn-container').classList.remove('hidden'); document.getElementById('floor-select').classList.remove('hidden'); document.getElementById('floor-select-label').classList.remove('hidden'); } else if(zone === '현장') { document.getElementById('tab-floor').className = "px-8 py-2.5 bg-emerald-500 text-white font-bold rounded-t-lg shadow-inner"; document.getElementById('fifo-btn-container').classList.add('hidden'); document.getElementById('floor-select').classList.add('hidden'); document.getElementById('floor-select-label').classList.add('hidden'); } renderMap(); }
        function renderMap() { const floor = document.getElementById('floor-select').value; const vContainer = document.getElementById('vertical-racks'); const hContainer = document.getElementById('horizontal-rack'); const occMap = {}; const palletMap = {}; globalOccupancy.forEach(item => { occMap[item.location_id] = true; palletMap[item.location_id] = (palletMap[item.location_id] || 0) + (item.pallet_count || 1); }); let vHtml = ''; hContainer.innerHTML = ''; if(currentZone === '현장') { document.getElementById('aisle-text').classList.add('hidden'); vHtml += `<div class="w-full min-w-[700px]">`; layoutFloor.forEach(col => { if(col.aisle) { vHtml += `<div class="w-full h-10 bg-yellow-50/50 flex items-center justify-center border-y-2 border-yellow-300 shadow-inner my-6 rounded-lg"><span class="text-yellow-600 font-black tracking-widest text-sm">${col.text}</span></div>`; } else { let colorClass = col.id === 'FL-C' ? 'text-indigo-800 bg-indigo-50 border-indigo-200' : 'text-orange-800 bg-orange-50 border-orange-200'; vHtml += `<div class="mb-4 bg-white p-5 rounded-2xl shadow-md border border-slate-200"><div class="text-lg font-black ${colorClass} p-3 rounded-lg border mb-4 shadow-sm inline-block">${col.title}</div><div class="grid grid-cols-10 gap-3">`; for (let r = 1; r <= col.cols; r++) { let dbId = `${col.id}-${r.toString().padStart(2, '0')}`; let searchId = dbId; let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === dbId) cellState = 'cell-active'; let pCount = palletMap[searchId] || 0; let badge = (pCount > 1) ? `<div class="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-md z-10 animate-bounce">${pCount}P</div>` : ''; vHtml += `<div id="cell-${dbId}" onclick="clickCell('${dbId}', '${searchId}')" class="h-16 rounded-xl border-2 flex flex-col items-center justify-center text-[11px] font-black cursor-pointer rack-cell ${cellState} shadow-sm hover:scale-105 transition-all">${badge}<span class="${hasItem?'text-slate-700':'text-slate-400'}">${r}번 칸</span></div>`; } vHtml += `</div></div>`; } }); vHtml += `</div>`; vContainer.innerHTML = vHtml; return; } document.getElementById('aisle-text').classList.remove('hidden'); document.getElementById('aisle-text').innerText = "통로 (Aisle)"; const activeLayout = currentZone === '실온' ? layoutRoom : layoutCold; activeLayout.forEach(col => { if (col.aisle) { vHtml += `<div class="w-14 h-[420px] bg-yellow-50/50 flex flex-col items-center justify-center border-x-2 border-yellow-300 shadow-inner rounded-sm mx-1"><span class="text-yellow-600 font-black tracking-widest text-xs" style="writing-mode: vertical-rl;">통로</span></div>`; } else if (col.gap) { vHtml += `<div class="w-4"></div>`; } else { vHtml += `<div class="flex flex-col w-14 space-y-1 justify-end"><div class="text-center font-black text-2xl text-slate-800 pb-2">${col.id}</div>`; for (let r = col.cols; r >= 1; r--) { let dbId = `${col.id}-${r.toString().padStart(2, '0')}`; let displayId = `${col.id}${r}`; let searchId = floor === "1" ? dbId : `${dbId}-2F`; let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === displayId) cellState = 'cell-active'; let pCount = palletMap[searchId] || 0; let badge = (pCount > 1) ? `<div class="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] font-black px-1 rounded-full shadow-sm z-10">${pCount}P</div>` : ''; vHtml += `<div id="cell-${displayId}" onclick="clickCell('${displayId}', '${searchId}')" class="h-8 rounded-[3px] flex items-center justify-center text-[10px] font-bold cursor-pointer rack-cell ${cellState} shadow-sm">${badge}${displayId}</div>`; } vHtml += `</div>`; } }); vContainer.innerHTML = vHtml; let hHtml = `<div class="flex space-x-1">`; let hPrefix = currentZone === '실온' ? 'K' : 'I'; let hCols = currentZone === '실온' ? 10 : 8; for (let c = hCols; c >= 1; c--) { let displayId = `${hPrefix}${c}`; let dbId = `${hPrefix}-${c.toString().padStart(2, '0')}`; let searchId = floor === "1" ? dbId : `${dbId}-2F`; let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === displayId) cellState = 'cell-active'; let pCount = palletMap[searchId] || 0; let badge = (pCount > 1) ? `<div class="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] font-black px-1 rounded-full shadow-sm z-10">${pCount}P</div>` : ''; hHtml += `<div id="cell-${displayId}" onclick="clickCell('${displayId}', '${searchId}')" class="w-14 h-10 rounded-[3px] flex items-center justify-center text-[11px] font-bold cursor-pointer rack-cell ${cellState} shadow-sm">${badge}${displayId}</div>`; } hHtml += `</div>`; hContainer.innerHTML = hHtml; }
        
        async function clickCell(displayId, searchId) { selectedCellId = displayId; if(!searchId) { const floor = document.getElementById('floor-select').value; const baseId = displayId.replace(/([A-Z])([0-9]+)/, (m, p1, p2) => `${p1}-${p2.padStart(2, '0')}`); searchId = floor === "1" ? baseId : `${baseId}-2F`; } renderMap(); const panel = document.getElementById('info-panel'); const floorName = currentZone === '현장' ? '생산현장' : document.getElementById('floor-select').options[document.getElementById('floor-select').selectedIndex].text; const items = globalOccupancy.filter(x => x.location_id === searchId); let panelHtml = `<div class="bg-indigo-50 p-4 rounded-lg border border-indigo-200 mb-4"><div class="flex justify-between items-start"><div><div class="text-[10px] text-indigo-500 font-bold mb-1">선택된 위치</div><div class="text-3xl font-black text-indigo-900">${displayId}</div></div><div class="text-right"><span class="inline-block bg-white text-indigo-700 text-xs font-bold px-2 py-1 rounded shadow-sm border border-indigo-100">${floorName}</span></div></div></div>`; if(items.length > 0) { panelHtml += `<div class="mb-2 text-xs font-bold text-slate-500">적재 목록 (${items.length}건)</div>`; items.forEach(item => { let dateHtml = item.production_date ? `<div class="text-xs text-rose-600 font-bold mt-1">산란일: ${item.production_date}</div>` : ''; let pInfo = productMaster.find(p => p.item_name === item.item_name && p.supplier === item.remarks); let cost = pInfo ? (pInfo.unit_price * item.quantity).toLocaleString() : '0'; let adjustBtn = isAdmin ? `<button onclick="processAdjust('${item.id}', '${item.item_name}', ${item.quantity}, '${searchId}')" class="text-[9px] text-slate-400 font-normal underline hover:text-indigo-600 ml-2">수량 보정</button>` : ''; let palletDisplay = item.pallet_count ? `<span class="bg-rose-100 text-rose-700 px-1.5 py-0.5 rounded text-[10px] ml-1 font-black">${item.pallet_count} P</span>` : ''; panelHtml += `<div class="bg-white border border-slate-200 rounded-lg p-3 shadow-sm mb-3"><div class="flex justify-between items-start mb-2"><div><span class="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">${item.category}</span><div class="font-black text-sm text-slate-800 mt-1">${item.item_name}${adjustBtn}</div><div class="text-[10px] text-slate-400 font-bold text-rose-600">입고처: ${item.remarks||'기본'} (총액: ${cost}원)</div></div><div class="text-right"><div class="text-sm font-bold text-indigo-600">${item.quantity} 박스 ${palletDisplay}</div></div></div>${dateHtml}<div class="flex space-x-2 mt-3 border-t pt-2"><button onclick="processTransfer('${item.id}', '${item.item_name}', ${item.quantity}, ${item.pallet_count||1}, '${searchId}')" class="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-200 py-1.5 rounded text-[11px] font-bold transition-colors">위치 이동</button><button onclick="processOutbound('${item.id}', '${item.item_name}', ${item.quantity}, ${item.pallet_count||1}, '${searchId}')" class="flex-1 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 py-1.5 rounded text-[11px] font-bold transition-colors">선택 출고</button></div></div>`; }); } else { panelHtml += `<div class="text-center text-slate-400 py-6 bg-slate-50 rounded-lg border border-dashed border-slate-300 mb-4"><b class="text-emerald-600 inline-block">비어있습니다 (Empty)</b></div>`; } const catOptions = [...new Set(productMaster.map(p => p.category))].map(c => `<option value="${c}">${c}</option>`).join(''); panelHtml += `<div class="mt-4 pt-4 border-t border-slate-200"><h3 class="text-sm font-black text-slate-700 mb-3 flex items-center"><svg class="w-4 h-4 mr-1 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> 신규 입고</h3><div class="space-y-2"><div class="flex space-x-2"><div class="w-1/3"><label class="block text-[10px] font-bold text-slate-400 mb-1">카테고리</label><select id="in-cat" onchange="updateProductDropdown()" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"><option value="">선택</option>${catOptions}</select></div><div class="w-2/3"><label class="block text-[10px] font-bold text-slate-400 mb-1">품목명</label><select id="in-item" onchange="updateSupplierDropdown()" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"><option value="">선택 대기</option></select></div></div><div class="flex space-x-2"><div class="w-1/2"><label class="block text-[10px] font-bold text-rose-500 mb-1">입고처 (단가 매칭용)</label><select id="in-supplier" class="w-full border border-rose-300 bg-rose-50 rounded p-1.5 text-xs font-bold"><option value="">품목 선택</option></select></div><div class="w-1/2"><label class="block text-[10px] font-bold text-slate-400 mb-1">산란일</label><input type="date" id="in-date" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"></div></div><div class="flex space-x-2"><div class="w-1/2"><label class="block text-[10px] font-bold text-slate-400 mb-1">박스 수량</label><input type="number" id="in-qty" value="1" min="1" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold text-indigo-700"></div><div class="w-1/2"><label class="block text-[10px] font-bold text-rose-500 mb-1">적재 파레트 수 (부피)</label><input type="number" id="in-pallet" value="1.0" min="0.1" step="0.1" class="w-full border border-rose-300 bg-rose-50 rounded p-1.5 text-xs font-bold text-rose-700"></div></div><button onclick="processInbound('${searchId}')" class="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-2 rounded-lg font-black shadow-md transition-colors text-sm mt-1">입고 처리</button></div></div>`; let locHistory = globalHistory.filter(h => h.location_id === searchId).sort((a,b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 5); let histHtml = '<div class="mt-6 pt-4 border-t border-slate-200"><h3 class="text-sm font-black text-slate-700 mb-3 flex items-center"><svg class="w-4 h-4 mr-1 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> 최근 내역 (최대 5건)</h3><div class="space-y-2">'; if(locHistory.length > 0) { locHistory.forEach(h => { let actionColor = h.action_type === '입고' ? 'text-emerald-600' : (h.action_type === '출고' ? 'text-rose-600' : 'text-blue-600'); let dateStr = new Date(h.created_at).toLocaleString('ko-KR', {month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit'}); histHtml += `<div class="bg-white p-2 border border-slate-200 rounded text-xs shadow-sm"><span class="font-bold ${actionColor}">[${h.action_type}]</span> <span class="text-slate-500">${dateStr}</span><br><span class="font-bold text-slate-700">${h.item_name} <span class="text-slate-400">(${h.quantity}박스 / ${h.pallet_count||1}P)</span></span></div>`; }); } else { histHtml += `<div class="text-xs text-slate-400 text-center py-4">기록이 없습니다.</div>`; } histHtml += '</div></div>'; panelHtml += histHtml; panel.innerHTML = panelHtml; }
        ...
        function generateAllLocations() { let locs = []; layoutRoom.forEach(col => { if(col.cols) { for(let r=1; r<=col.cols; r++) { locs.push({ zone: '창고-실온(1F)', id: `${col.id}-${r.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-실온(2F)', id: `${col.id}-${r.toString().padStart(2, '0')}-2F` }); } } }); for(let c=1; c<=10; c++) { locs.push({ zone: '창고-실온(가로K-1F)', id: `K-${c.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-실온(가로K-2F)', id: `K-${c.toString().padStart(2, '0')}-2F` }); } layoutCold.forEach(col => { if(col.cols) { for(let r=1; r<=col.cols; r++) { locs.push({ zone: '창고-냉장(1F)', id: `${col.id}-${r.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-냉장(2F)', id: `${col.id}-${r.toString().padStart(2, '0')}-2F` }); } } }); for(let c=1; c<=8; c++) { locs.push({ zone: '창고-냉장(가로I-1F)', id: `I-${c.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-냉장(가로I-2F)', id: `I-${c.toString().padStart(2, '0')}-2F` }); } for(let c=1; c<=20; c++) locs.push({ zone: '생산현장-냉장', id: `FL-C-${c.toString().padStart(2, '0')}` }); for(let c=1; c<=20; c++) locs.push({ zone: '생산현장-실온', id: `FL-R-${c.toString().padStart(2, '0')}` }); return locs; }
        function exportPhysicalCountExcel() { if(!confirm("모든 렉을 실사용 엑셀로 다운로드합니다.")) return; const allLocs = generateAllLocations(); const wsData = []; allLocs.forEach(loc => { const items = globalOccupancy.filter(x => x.location_id === loc.id); if (items.length > 0) { items.forEach(item => { wsData.push({ "구역": loc.zone, "렉 위치": loc.id, "품목명": item.item_name, "입고처": item.remarks || "", "전산 수량 (박스)": item.quantity, "파레트 부피": item.pallet_count || 1, "실제 수량 (수기작성)": "              ", "차이 사유 / 비고": "" }); }); } else { wsData.push({ "구역": loc.zone, "렉 위치": loc.id, "품목명": "(빈 칸)", "입고처": "", "전산 수량 (박스)": 0, "파레트 부피": 0, "실제 수량 (수기작성)": "              ", "차이 사유 / 비고": "" }); } }); const wb = XLSX.utils.book_new(); const ws = XLSX.utils.json_to_sheet(wsData); ws['!cols'] = [ {wch: 15}, {wch: 15}, {wch: 25}, {wch: 15}, {wch: 15}, {wch: 12}, {wch: 25}, {wch: 30} ]; XLSX.utils.book_append_sheet(wb, ws, "현장 재고실사 양식"); XLSX.writeFile(wb, "한스팜_재고실사양식(수기작성용).xlsx"); }
        
        function renderProductMaster() { document.getElementById('pm-list').innerHTML = productMaster.map(p => { let delBtn = isAdmin ? `<button onclick="deleteProduct('${p.item_name}', '${p.supplier}')" class="text-rose-500 hover:bg-rose-100 p-1.5 rounded transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>` : ''; return `<div class="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-200"><div><span class="text-[10px] bg-slate-200 text-slate-600 px-2 py-0.5 rounded mr-1">${p.category}</span><span class="font-bold text-sm text-slate-800">${p.item_name}</span> <span class="text-xs font-bold text-rose-600">[${p.supplier}]</span><div class="text-[10px] text-slate-500 mt-1">소모량: ${p.daily_usage}개 | 단가: ${p.unit_price.toLocaleString()}원</div></div><div class="flex space-x-1"><button onclick="editProductSetup('${p.category}', '${p.item_name}', '${p.supplier}', ${p.daily_usage}, ${p.unit_price})" class="text-blue-500 hover:bg-blue-100 p-1.5 rounded transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></button>${delBtn}</div></div>`; }).join(''); }
        function editProductSetup(cat, name, supplier, usage, price) { editingProductOriginalName = name; editingProductOriginalSupplier = supplier; document.getElementById('pm-cat').value = cat; document.getElementById('pm-name').value = name; document.getElementById('pm-supplier').value = supplier; document.getElementById('pm-usage').value = usage; document.getElementById('pm-price').value = price; document.getElementById('pm-form-title').innerText = "기존 품목 수정 모드"; document.getElementById('pm-submit-btn').innerText = "✅ 변경사항 저장"; document.getElementById('pm-submit-btn').classList.replace('bg-indigo-600', 'bg-emerald-600'); document.getElementById('pm-submit-btn').classList.replace('hover:bg-indigo-700', 'hover:bg-emerald-700'); document.getElementById('pm-cancel-btn').classList.remove('hidden'); }
        function cancelEdit() { editingProductOriginalName = null; editingProductOriginalSupplier = null; document.getElementById('pm-cat').value = ''; document.getElementById('pm-name').value = ''; document.getElementById('pm-supplier').value = ''; document.getElementById('pm-usage').value = '0'; document.getElementById('pm-price').value = '0'; document.getElementById('pm-form-title').innerText = "신규 품목 추가"; document.getElementById('pm-submit-btn').innerText = "DB에 등록하기"; document.getElementById('pm-submit-btn').classList.replace('bg-emerald-600', 'bg-indigo-600'); document.getElementById('pm-submit-btn').classList.replace('hover:bg-emerald-700', 'hover:bg-indigo-700'); document.getElementById('pm-cancel-btn').classList.add('hidden'); }
        async function submitProduct() { const cat = document.getElementById('pm-cat').value.trim(); const name = document.getElementById('pm-name').value.trim(); const supplier = document.getElementById('pm-supplier').value.trim() || '기본입고처'; const usage = parseInt(document.getElementById('pm-usage').value) || 0; const price = parseInt(document.getElementById('pm-price').value) || 0; if(!cat || !name) return alert("카테고리와 품목명은 필수입니다."); try { if(editingProductOriginalName) { await fetch(`/api/products?old_name=${encodeURIComponent(editingProductOriginalName)}&old_supplier=${encodeURIComponent(editingProductOriginalSupplier)}`, { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify({category: cat, item_name: name, supplier: supplier, daily_usage: usage, unit_price: price}) }); alert("수정 완료"); cancelEdit(); } else { await fetch('/api/products', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({category: cat, item_name: name, supplier: supplier, daily_usage: usage, unit_price: price}) }); alert("등록 완료"); document.getElementById('pm-name').value = ''; document.getElementById('pm-supplier').value = ''; } load(); } catch(e) { alert("서버 통신 실패"); } }
        ...
        function exportProductsExcel() { let wsData = productMaster.length === 0 ? [{ "카테고리": "", "품목명": "", "입고처(공급사)": "", "일간소모량": "", "단가(비용)": "" }] : productMaster.map(p => ({ "카테고리": p.category, "품목명": p.item_name, "입고처(공급사)": p.supplier, "일간소모량": p.daily_usage, "단가(비용)": p.unit_price })); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(wsData), "품목마스터"); XLSX.writeFile(wb, "품목마스터_양식.xlsx"); }
        ...
        function importExcel(e) { const file = e.target.files[0]; if(!file) return; const reader = new FileReader(); reader.onload = async function(ev) { const data = new Uint8Array(ev.target.result); const workbook = XLSX.read(data, {type: 'array'}); const json = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]]); if(json.length > 0) { await fetch('/api/inbound_batch', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(json) }); alert("업로드 완료!"); load(); } }; reader.readAsArrayBuffer(file); e.target.value = ''; }
        
        async function processInbound(locId) { const cat = document.getElementById('in-cat').value; const item = document.getElementById('in-item').value; const qty = document.getElementById('in-qty').value; const date = document.getElementById('in-date').value; const supplier = document.getElementById('in-supplier').value; const pallet = parseFloat(document.getElementById('in-pallet').value) || 1.0; if(!cat || !item || !qty) return alert("필수값 입력 요망"); try { await fetch('/api/inbound', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ location_id: locId, category: cat, item_name: item, quantity: parseInt(qty), pallet_count: pallet, production_date: date || null, remarks: supplier }) }); alert("입고 완료!"); load(); } catch(e) {} }
        async function processOutbound(invId, itemName, maxQty, currentPallet, locId) { const qtyStr = prompt(`[${itemName}] 소진할 수량을 입력하세요. (최대 ${maxQty}박스)`, maxQty); if(!qtyStr) return; const qty = parseInt(qtyStr); if(isNaN(qty) || qty <= 0 || qty > maxQty) return alert("잘못된 수량"); const outPallet = (qty / maxQty) * currentPallet; try { await fetch('/api/outbound', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ inventory_id: invId, location_id: locId, item_name: itemName, quantity: qty, pallet_count: outPallet }) }); alert("출고 완료!"); load(); } catch(e) {} }
        async function processTransfer(invId, itemName, maxQty, currentPallet, fromLoc) { const qtyStr = prompt(`[${itemName}] 이동시킬 수량을 입력하세요. (최대 ${maxQty}박스)`, maxQty); if(!qtyStr) return; const qty = parseInt(qtyStr); if(isNaN(qty) || qty <= 0 || qty > maxQty) return alert("잘못된 수량"); const toLoc = prompt(`[${itemName}] ${qty}박스 이동할 목적지를 입력하세요\\n(창고 예시: B-02-1F, 현장 예시: FL-C-01)`, "FL-C-01"); if(!toLoc) return; const movePallet = (qty / maxQty) * currentPallet; try { await fetch('/api/transfer', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ inventory_id: invId, from_location: fromLoc, to_location: toLoc.toUpperCase(), item_name: itemName, quantity: qty, pallet_count: movePallet }) }); alert("이동 완료!"); load(); } catch(e) {} }
...
async def add_product(data: ProductData):
    async with httpx.AsyncClient() as client:
        res = await client.post(f"{SUPABASE_URL}/rest/v1/products", json={"category": data.category, "item_name": data.item_name, "supplier": data.supplier, "daily_usage": data.daily_usage, "unit_price": data.unit_price}, headers=HEADERS)
...
async def update_product(old_name: str, old_supplier: str, data: ProductData):
    async with httpx.AsyncClient() as client:
        res = await client.patch(f"{SUPABASE_URL}/rest/v1/products?item_name=eq.{old_name}&supplier=eq.{old_supplier}", json={"category": data.category, "item_name": data.item_name, "supplier": data.supplier, "daily_usage": data.daily_usage, "unit_price": data.unit_price}, headers=HEADERS)
        if res.status_code in [200, 201, 204]: return {"status": "success"}
        return {"status": "error", "message": res.text}
...
async def products_batch(rows: List[dict]):
    async with httpx.AsyncClient() as client:
        payloads = [{"category": r.get("카테고리", "미분류"), "item_name": r.get("품목명", ""), "supplier": r.get("입고처(공급사)", "기본입고처"), "daily_usage": int(r.get("일간소모량", 0)), "unit_price": int(r.get("단가(비용)", 0))} for r in rows if r.get("품목명")]
        if payloads:
            headers = HEADERS.copy(); headers["Prefer"] = "resolution=ignore-duplicates"
...
async def transfer_stock(data: TransferData):
    async with httpx.AsyncClient() as client:
        ...
        if data.quantity >= current_qty: await client.patch(f"{SUPABASE_URL}/rest/v1/inventory_v2?id=eq.{data.inventory_id}", json={"location_id": data.to_location}, headers=HEADERS)
            ...
            new_row = item_data.copy(); del new_row['id']; del new_row['created_at']; new_row['location_id'] = data.to_location; new_row['quantity'] = data.quantity; new_row['pallet_count'] = data.pallet_count
            await client.post(f"{SUPABASE_URL}/rest/v1/inventory_v2", json=new_row, headers=HEADERS)
        await client.post(f"{SUPABASE_URL}/rest/v1/history_log", json={"location_id": data.to_location, "action_type": "이동", "item_name": data.item_name, "quantity": data.quantity, "remarks": f"From {data.from_location}", "pallet_count": data.pallet_count}, headers=HEADERS)
        return {"status": "success"}
...
async def inbound_batch(rows: List[dict]):
    async with httpx.AsyncClient() as client:
        payloads = [{"location_id": r.get("렉 위치", ""), "category": r.get("카테고리", "미분류"), "item_name": r.get("품목명", ""), "quantity": int(r.get("수량", 1)), "pallet_count": float(r.get("파레트부피", 1.0)), "production_date": r.get("산란일", None) if r.get("산란일") else None, "remarks": r.get("입고처(비고)", "")} for r in rows]
