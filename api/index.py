...
<body class="font-sans h-screen flex overflow-hidden text-slate-800 selection:bg-indigo-200">
    ...
    <script>
        ...
        // ==========================================
        // 📋 실사 양식 다운로드 (전체 렉 자동생성 정렬본)
        // ==========================================
        function generateAllLocations() {
            let locs = [];
            // 실온 정렬
            layoutRoom.forEach(col => { if(col.cols) { for(let r=1; r<=col.cols; r++) { locs.push({ zone: '창고-실온(1F)', id: `${col.id}-${r.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-실온(2F)', id: `${col.id}-${r.toString().padStart(2, '0')}-2F` }); } } });
            for(let c=1; c<=10; c++) { locs.push({ zone: '창고-실온(가로K-1F)', id: `K-${c.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-실온(가로K-2F)', id: `K-${c.toString().padStart(2, '0')}-2F` }); }
            // 냉장 정렬
            layoutCold.forEach(col => { if(col.cols) { for(let r=1; r<=col.cols; r++) { locs.push({ zone: '창고-냉장(1F)', id: `${col.id}-${r.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-냉장(2F)', id: `${col.id}-${r.toString().padStart(2, '0')}-2F` }); } } });
            for(let c=1; c<=8; c++) { locs.push({ zone: '창고-냉장(가로I-1F)', id: `I-${c.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-냉장(가로I-2F)', id: `I-${c.toString().padStart(2, '0')}-2F` }); }
            // 현장 정렬
            for(let c=1; c<=20; c++) locs.push({ zone: '생산현장-냉장', id: `FL-C-${c.toString().padStart(2, '0')}` });
            for(let c=1; c<=20; c++) locs.push({ zone: '생산현장-실온', id: `FL-R-${c.toString().padStart(2, '0')}` });
            return locs;
        }

        function exportPhysicalCountExcel() {
            if(!confirm("창고 및 현장의 모든 렉 넘버가 미리 적힌 '업로드/실사 겸용 양식'을 다운로드합니다.\\n(이 파일에 수량을 적어서 '엑셀 대량 입고' 버튼으로 올리시면 됩니다.)")) return;
            const allLocs = generateAllLocations(); const wsData = [];
            allLocs.forEach(loc => {
                const items = globalOccupancy.filter(x => x.location_id === loc.id);
                if (items.length > 0) { items.forEach(item => { wsData.push({ "구역": loc.zone, "렉 위치": loc.id, "카테고리": item.category, "품목명": item.item_name, "입고처(비고)": item.remarks || "", "수량(EA)": item.quantity, "산란일/입고일": item.production_date || "" }); }); } 
                else { wsData.push({ "구역": loc.zone, "렉 위치": loc.id, "카테고리": "", "품목명": "", "입고처(비고)": "", "수량(EA)": "", "산란일/입고일": "" }); }
            });
            const wb = XLSX.utils.book_new(); 
            const ws = XLSX.utils.json_to_sheet(wsData);
            ws['!cols'] = [ {wch: 15}, {wch: 15}, {wch: 15}, {wch: 25}, {wch: 15}, {wch: 12}, {wch: 15} ];
            XLSX.utils.book_append_sheet(wb, ws, "재고입력_및_실사양식"); 
            
            // 💡 품목 참조 탭 추가 (타이핑 시간 단축)
            ...
            XLSX.utils.book_append_sheet(wb, wsRef, "품목DB(복사참조용)");
        ...
        function renderProductMaster() { 
            document.getElementById('pm-list').innerHTML = productMaster.map(p => { 
                let delBtn = isAdmin ? `<button onclick="deleteProduct('${p.item_name}', '${p.supplier}')" class="text-rose-500 hover:bg-rose-100 p-1.5 rounded transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>` : '';
                return `<div class="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-200">
                    <div><span class="text-[10px] bg-slate-200 text-slate-600 px-2 py-0.5 rounded mr-1">${p.category}</span><span class="font-bold text-sm text-slate-800">${p.item_name}</span> <span class="text-xs font-bold text-rose-600">[${p.supplier}]</span>
                    <div class="text-[10px] text-slate-500 mt-1">1P기준: <b class="text-indigo-600">${p.pallet_ea} EA</b> | 소모량: ${p.daily_usage} EA | 단가: ${p.unit_price.toLocaleString()}원</div></div>
                    <div class="flex space-x-1">
                        <button onclick="editProductSetup('${p.category}', '${p.item_name}', '${p.supplier}', ${p.daily_usage}, ${p.unit_price}, ${p.pallet_ea})" class="text-blue-500 hover:bg-blue-100 p-1.5 rounded transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></button>
                        ${delBtn}
                    </div>
                </div>`; 
            }).join(''); 
        ...
        function editProductSetup(cat, name, supplier, usage, price, ea) {
            editingProductOriginalName = name; editingProductOriginalSupplier = supplier;
            document.getElementById('pm-cat').value = cat; document.getElementById('pm-name').value = name; document.getElementById('pm-supplier').value = supplier; document.getElementById('pm-usage').value = usage; document.getElementById('pm-price').value = price; document.getElementById('pm-pallet-ea').value = ea || 1;
            document.getElementById('pm-form-title').innerText = "기존 품목 수정 모드"; document.getElementById('pm-submit-btn').innerText = "✅ 변경사항 저장"; document.getElementById('pm-submit-btn').classList.replace('bg-indigo-600', 'bg-emerald-600'); document.getElementById('pm-submit-btn').classList.replace('hover:bg-indigo-700', 'hover:bg-emerald-700'); document.getElementById('pm-cancel-btn').classList.remove('hidden');
        }
        function cancelEdit() {
            editingProductOriginalName = null; editingProductOriginalSupplier = null;
            document.getElementById('pm-cat').value = ''; document.getElementById('pm-name').value = ''; document.getElementById('pm-supplier').value = ''; document.getElementById('pm-usage').value = '0'; document.getElementById('pm-price').value = '0'; document.getElementById('pm-pallet-ea').value = '1';
            document.getElementById('pm-form-title').innerText = "신규 품목 추가"; document.getElementById('pm-submit-btn').innerText = "DB에 등록하기"; document.getElementById('pm-submit-btn').classList.replace('bg-emerald-600', 'bg-indigo-600'); document.getElementById('pm-submit-btn').classList.replace('hover:bg-emerald-700', 'hover:bg-indigo-700'); document.getElementById('pm-cancel-btn').classList.add('hidden');
        }

        async function submitProduct() {
            const cat = document.getElementById('pm-cat').value.trim(); const name = document.getElementById('pm-name').value.trim(); const supplier = document.getElementById('pm-supplier').value.trim() || '기본입고처'; const usage = parseInt(document.getElementById('pm-usage').value) || 0; const price = parseInt(document.getElementById('pm-price').value) || 0; const ea = parseInt(document.getElementById('pm-pallet-ea').value) || 1;
            if(!cat || !name) return alert("카테고리와 품목명은 필수입니다.");
            try { 
                ...
                else { await fetch('/api/products', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({category: cat, item_name: name, supplier: supplier, daily_usage: usage, unit_price: price, pallet_ea: ea}) }); alert("등록 완료"); document.getElementById('pm-name').value = ''; document.getElementById('pm-supplier').value = ''; }
                load(); 
        ...
        function exportProductsExcel() { let wsData = productMaster.length === 0 ? [{ "카테고리": "", "품목명": "", "입고처(공급사)": "", "일간소모량(EA)": "", "단가(비용)": "", "1P기준수량(EA)": "" }] : productMaster.map(p => ({ "카테고리": p.category, "품목명": p.item_name, "입고처(공급사)": p.supplier, "일간소모량(EA)": p.daily_usage, "단가(비용)": p.unit_price, "1P기준수량(EA)": p.pallet_ea })); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(wsData), "품목마스터"); XLSX.writeFile(wb, "품목마스터_양식.xlsx"); }
        ...
        async function clickCell(displayId, searchId) {
            ...
            if(items.length > 0) {
                ...
                items.forEach(item => {
                    ...
                    let palletDisplay = item.pallet_count ? `<span class="bg-rose-100 text-rose-700 px-1.5 py-0.5 rounded text-[10px] ml-1 font-black">${item.pallet_count.toFixed(1)} P</span>` : '';
                    ...
                    panelHtml += `
                        <div class="bg-white border border-slate-200 rounded-lg p-3 shadow-sm mb-3">
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <div class="font-black text-sm text-slate-800 mt-1">${item.item_name}${adjustBtn}</div><div class="text-[10px] text-slate-400 font-bold text-rose-600">입고처: ${item.remarks||'기본'} (총액: ${cost}원)</div></div>
                                <div class="text-right"><div class="text-sm font-bold text-indigo-600">${item.quantity.toLocaleString()} EA ${palletDisplay}</div></div>
                            </div>
                            ${dateHtml}
                            <div class="flex space-x-2 mt-3 border-t pt-2">
                                <button onclick="processTransfer('${item.id}', '${item.item_name}', ${item.quantity}, ${item.pallet_count||1}, '${searchId}')" class="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-200 py-1.5 rounded text-[11px] font-bold transition-colors">위치 이동</button>
                                <button onclick="processOutbound('${item.id}', '${item.item_name}', ${item.quantity}, ${item.pallet_count||1}, '${searchId}')" class="flex-1 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 py-1.5 rounded text-[11px] font-bold transition-colors">선택 출고</button>
                            </div>
                        </div>`;
                });
            ...
            panelHtml += `
                <div class="mt-4 pt-4 border-t border-slate-200">
                    ...
                    <div class="space-y-2">
                        <div class="flex space-x-2"><div class="w-1/3"><label class="block text-[10px] font-bold text-slate-400 mb-1">카테고리</label><select id="in-cat" onchange="updateProductDropdown()" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"><option value="">선택</option>${catOptions}</select></div><div class="w-2/3"><label class="block text-[10px] font-bold text-slate-400 mb-1">품목명</label><select id="in-item" onchange="updateSupplierDropdown()" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"><option value="">선택 대기</option></select></div></div>
                        <div class="flex space-x-2"><div class="w-1/2"><label class="block text-[10px] font-bold text-rose-500 mb-1">입고처 (단가 매칭용)</label><select id="in-supplier" class="w-full border border-rose-300 bg-rose-50 rounded p-1.5 text-xs font-bold"><option value="">품목 선택</option></select></div><div class="w-1/2"><label class="block text-[10px] font-bold text-slate-400 mb-1">${dateLabel}</label><input type="date" id="in-date" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"></div></div>
                        <div><label class="block text-[10px] font-bold text-indigo-600 mb-1">수량 (EA)</label><input type="number" id="in-qty" value="1" min="1" class="w-full border border-indigo-300 bg-indigo-50 rounded p-1.5 text-xs font-black text-indigo-700"></div>
                        <p class="text-[9px] text-slate-400 text-center mb-1">* 파레트 부피(P)는 품목 마스터 정보를 바탕으로 자동 계산됩니다.</p>
            ...
            if(locHistory.length > 0) { locHistory.forEach(h => { let actionColor = h.action_type === '입고' ? 'text-emerald-600' : (h.action_type === '출고' ? 'text-rose-600' : 'text-blue-600'); let dateStr = new Date(h.created_at).toLocaleString('ko-KR', {month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit'}); histHtml += `<div class="bg-white p-2 border border-slate-200 rounded text-xs shadow-sm"><span class="font-bold ${actionColor}">[${h.action_type}]</span> <span class="text-slate-500">${dateStr}</span><br><span class="font-bold text-slate-700">${h.item_name} <span class="text-slate-400">(${h.quantity}EA / ${h.pallet_count ? h.pallet_count.toFixed(1) : 1}P)</span></span></div>`; }); } else { histHtml += `<div class="text-xs text-slate-400 text-center py-4">기록이 없습니다.</div>`; }
            histHtml += '</div></div>'; panelHtml += histHtml; panel.innerHTML = panelHtml;
        }

        async function processInbound(locId) { 
            const cat = document.getElementById('in-cat').value; const item = document.getElementById('in-item').value; const qty = parseInt(document.getElementById('in-qty').value); 
            let date = document.getElementById('in-date').value; const supplier = document.getElementById('in-supplier').value; 
            if(!cat || !item || isNaN(qty)) return alert("필수값 입력 요망"); 
            
            // 날짜 안 넣었을 때 입고일이면 오늘 날짜 자동삽입
            if(!date && currentZone !== '냉장') { let t = new Date(); date = t.toISOString().split('T')[0]; }

            // 💡 파레트 부피 자동 계산
            let pInfo = productMaster.find(p => p.item_name === item && p.supplier === supplier);
            let palletEa = (pInfo && pInfo.pallet_ea > 0) ? pInfo.pallet_ea : 1;
            let autoPalletCount = qty / palletEa;

            try { await fetch('/api/inbound', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ location_id: locId, category: cat, item_name: item, quantity: qty, pallet_count: autoPalletCount, production_date: date || null, remarks: supplier }) }); alert("입고 완료!"); load(); } catch(e) {} 
        }

        async function processOutbound(invId, itemName, maxQty, currentPallet, locId) { 
            const qtyStr = prompt(`[${itemName}] 소진할 수량(EA)을 입력하세요. (최대 ${maxQty}EA)`, maxQty); if(!qtyStr) return; const qty = parseInt(qtyStr); if(isNaN(qty) || qty <= 0 || qty > maxQty) return alert("잘못된 수량"); 
            const outPallet = (qty / maxQty) * currentPallet; 
            try { await fetch('/api/outbound', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ inventory_id: invId, location_id: locId, item_name: itemName, quantity: qty, pallet_count: outPallet }) }); alert("출고 완료!"); load(); } catch(e) {} 
        }

        async function processTransfer(invId, itemName, maxQty, currentPallet, fromLoc) { 
            const qtyStr = prompt(`[${itemName}] 이동시킬 수량(EA)을 입력하세요. (최대 ${maxQty}EA)`, maxQty); if(!qtyStr) return; const qty = parseInt(qtyStr); if(isNaN(qty) || qty <= 0 || qty > maxQty) return alert("잘못된 수량"); 
            const toLoc = prompt(`[${itemName}] ${qty}EA를 이동할 목적지를 입력하세요\\n(창고 예시: B-02-1F, 현장 예시: FL-C-01)`, "FL-C-01"); if(!toLoc) return; 
            const movePallet = (qty / maxQty) * currentPallet; 
            try { await fetch('/api/transfer', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ inventory_id: invId, from_location: fromLoc, to_location: toLoc.toUpperCase(), item_name: itemName, quantity: qty, pallet_count: movePallet }) }); alert("이동 완료!"); load(); } catch(e) {} 
        }
        ...
        function exportExcel() { let wsData = []; if(globalOccupancy.length === 0) { alert("재고가 없어 양식만 다운로드합니다."); wsData = [{ "렉 위치": "", "카테고리": "", "품목명": "", "수량(EA)": "", "단가(원)": "", "총비용(원)": "", "산란일/입고일": "", "입고처(비고)": "", "파레트부피": "" }]; } else { wsData = globalOccupancy.map(item => { let pInfo = productMaster.find(p => p.item_name === item.item_name && p.supplier === item.remarks); let price = pInfo ? pInfo.unit_price : 0; return { "렉 위치": item.location_id, "카테고리": item.category, "품목명": item.item_name, "수량(EA)": item.quantity, "단가(원)": price, "총비용(원)": price * item.quantity, "산란일/입고일": item.production_date || "", "입고처(비고)": item.remarks || "", "파레트부피": item.pallet_count || 1 }; }); } const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(wsData), "재고 및 자산"); XLSX.writeFile(wb, "한스팜_재고자산현황.xlsx"); }
...
async def products_batch(rows: List[dict]):
    async with httpx.AsyncClient() as client:
        payloads = [{"category": r.get("카테고리", "미분류"), "item_name": r.get("품목명", ""), "supplier": r.get("입고처(공급사)", "기본입고처"), "daily_usage": int(r.get("일간소모량(EA)", 0)), "unit_price": int(r.get("단가(비용)", 0)), "pallet_ea": int(r.get("1P기준수량(EA)", 1))} for r in rows if r.get("품목명")]
        if payloads:
...
async def inbound_batch(rows: List[dict]):
    async with httpx.AsyncClient() as client:
        ...
        for r_data in rows:
            name = r_data.get("품목명", "")
            if not name or name == "(빈 칸)": continue # 빈칸 스킵
            
            qty = int(r_data.get("수량(EA)", 0))
            supplier = r_data.get("입고처(비고)", "")
            
            # 파레트 부피 자동 계산
            p_info = next((p for p in pm_list if p["item_name"] == name and p["supplier"] == supplier), None)
            pallet_ea = p_info["pallet_ea"] if p_info and p_info["pallet_ea"] > 0 else 1
            calculated_pallet = qty / pallet_ea
            
            payloads.append({
                "location_id": r_data.get("렉 위치", ""),
                "category": r_data.get("카테고리", "미분류"),
                "item_name": name,
                "quantity": qty,
                "pallet_count": calculated_pallet,
                "production_date": r_data.get("산란일/입고일", None) if r_data.get("산란일/입고일") else None,
                "remarks": supplier
            })
