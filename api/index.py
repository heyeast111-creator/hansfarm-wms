...
<body class="font-sans h-screen flex overflow-hidden text-slate-800 selection:bg-indigo-200">
    ...
    <script>
        ...
        function generateLocations(zone) {
            let locs = [];
            if(zone === '실온') {
                layoutRoom.forEach(col => { if(col.cols) { for(let r=1; r<=col.cols; r++) { locs.push(`${col.id}-${r.toString().padStart(2, '0')}`); locs.push(`${col.id}-${r.toString().padStart(2, '0')}-2F`); } } });
                for(let c=1; c<=12; c++) { locs.push(`K-${c.toString().padStart(2, '0')}`); locs.push(`K-${c.toString().padStart(2, '0')}-2F`); }
            } else if(zone === '냉장') {
                layoutCold.forEach(col => { if(col.cols) { for(let r=1; r<=col.cols; r++) { locs.push(`${col.id}-${r.toString().padStart(2, '0')}`); locs.push(`${col.id}-${r.toString().padStart(2, '0')}-2F`); } } });
                for(let c=1; c<=8; c++) { locs.push(`I-${c.toString().padStart(2, '0')}`); locs.push(`I-${c.toString().padStart(2, '0')}-2F`); }
            } else if(zone === '현장') {
        ...
        function exportPhysicalCountExcel() {
            ...
            ['실온', '냉장', '현장'].forEach(zone => {
                ...
                locs.forEach(locId => {
                    ...
                    if (items.length > 0) { items.forEach(item => { wsData.push({ "구역": zone, "렉 위치": locId, "카테고리": item.category, "품목명": item.item_name, "입고처(비고)": item.remarks || "", "수량(EA)": item.quantity, "산란일/입고일": item.production_date || "" }); }); } 
                    else { wsData.push({ "구역": zone, "렉 위치": locId, "카테고리": "", "품목명": "", "입고처(비고)": "", "수량(EA)": "", "산란일/입고일": "" }); }
                });
                const ws = XLSX.utils.json_to_sheet(wsData); ws['!cols'] = [ {wch: 10}, {wch: 15}, {wch: 15}, {wch: 25}, {wch: 15}, {wch: 12}, {wch: 15} ]; XLSX.utils.book_append_sheet(wb, ws, zone); 
            });
            ...
            reader.onload = async function(ev) { 
                try {
                    ...
                    if(allRows.length > 0) { 
                        ...
                        if(result.status === 'success') { alert(`업로드 성공! 총 ${result.count}건의 데이터가 등록되었습니다.`); load(); }
                        else { alert(`업로드 실패: ${result.message}`); }
                    } else { alert("업로드할 데이터가 없습니다."); }
                } catch(err) { alert("엑셀 파일 처리 중 오류 발생! 양식을 확인해주세요."); console.error(err); }
            }; 
...
async def products_batch(rows: List[dict]):
    async with httpx.AsyncClient() as client:
        ...
        for r in rows:
            ...
            payloads.append({"category": str(r.get("카테고리", "미분류")).strip(), "item_name": name, "supplier": str(r.get("입고처(공급사)", "기본입고처")).strip() or "기본입고처", "daily_usage": usage, "unit_price": price, "pallet_ea": ea})
        if payloads:
...
async def inbound_batch(rows: List[dict]):
    async with httpx.AsyncClient() as client:
        ...
        for r_data in rows:
            ...
            qty_raw = str(r_data.get("수량(EA)", "0")).replace(',', '').strip()
            ...
            payloads.append({
                "location_id": str(r_data.get("렉 위치", "")).strip(), 
