        ...
        function exportProductsExcel() {
            try {
                ...
                if (productMaster.length === 0) {
                    ...
                    // 과거 데이터에 null값이 있어도 0이나 1로 안전하게 변환
        ...
        function renderProductMaster() { 
            document.getElementById('pm-list').innerHTML = productMaster.map(p => { 
                ...
                return `<div class="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-200">
                    <div><span class="text-[10px] bg-slate-200 text-slate-600 px-2 py-0.5 rounded mr-1">${p.category}</span><span class="font-bold text-sm text-slate-800">${p.item_name}</span> <span class="text-xs font-bold text-rose-600">[${p.supplier}]</span><div class="text-[10px] text-slate-500 mt-1">소모량: ${p.daily_usage}개 | 단가: ${p.unit_price.toLocaleString()}원</div></div>
                    <div class="flex space-x-1">
