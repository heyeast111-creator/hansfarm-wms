    ...
    <script>
        ...
        function exportProductsExcel() {
            try {
                ...
                if (!productMaster || productMaster.length === 0) {
                    wsData = [{ 
                        "카테고리": "", "품목명": "", "입고처(공급사)": "", "일간소모량(EA)": "", "단가(비용)": "", "1P기준수량(EA)": "" 
                    }];
                } else {
        ...
        function exportPhysicalCountExcel() {
            ...
            ['실온', '냉장', '현장'].forEach(zone => {
                ...
                locs.forEach(locId => {
                    ...
                    if (items.length > 0) { 
                        items.forEach(item => { 
                            wsData.push({ "구역": zone, "렉 위치": locId, "카테고리": item.category, "품목명": item.item_name, "입고처(비고)": item.remarks || "", "수량(EA)": item.quantity, "산란일/입고일": item.production_date || "" }); 
                        }); 
                    } else { 
                        wsData.push({ "구역": zone, "렉 위치": locId, "카테고리": "", "품목명": "", "입고처(비고)": "", "수량(EA)": "", "산란일/입고일": "" }); 
                    }
                });
                const ws = XLSX.utils.json_to_sheet(wsData); 
                ws['!cols'] = [ {wch: 10}, {wch: 15}, {wch: 15}, {wch: 25}, {wch: 15}, {wch: 12}, {wch: 15} ]; 
        ...
        function importExcel(e) { 
            ...
            reader.onload = async function(ev) { 
                try {
                    ...
                    if(allRows.length > 0) { 
                        await fetch('/api/inbound_batch', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(allRows) }); 
                        alert("다중 시트 재고 업로드 완료!"); load(); 
                    } else { alert("업로드할 데이터가 없습니다."); }
                } catch(err) { alert("엑셀 파일 처리 중 오류 발생!"); }
            }; 
        ...
        function renderSafetyStock() { 
            ...
            if(monitoredProducts.length === 0) { html = `<tr><td colspan="6" class="p-10 text-center text-slate-400 font-bold">일간 소모량이 등록된 품목이 없습니다.</td></tr>`; } 
                ...
                monitoredProducts.forEach(p => { 
                    ...
                    html += `<tr class="hover:bg-slate-50 transition-colors ${isDanger ? 'bg-rose-50/30' : ''}"><td class="p-4 text-slate-500 text-sm font-bold">${p.category}</td><td class="p-4 text-slate-800 font-black">${p.item_name} <span class="text-rose-600 text-xs">[${p.supplier}]</span></td><td class="p-4 text-right font-bold text-lg text-indigo-700">${totalQty.toLocaleString()}</td><td class="p-4 text-right font-bold text-slate-500">${p.daily_usage.toLocaleString()} / 일</td><td class="p-4 text-center"><div class="w-full bg-slate-200 rounded-full h-2.5 mb-1 max-w-[150px] mx-auto overflow-hidden"><div class="h-2.5 rounded-full ${isDanger ? 'bg-rose-500' : 'bg-emerald-500'}" style="width: ${Math.min((safeDaysLeft/targetDays)*100, 100)}%"></div></div><span class="text-xs font-bold ${isDanger ? 'text-rose-600' : 'text-slate-500'}">${safeDaysLeft.toFixed(1)} 일 버팀</span></td><td class="p-4 text-center">${statusHtml}</td></tr>`; 
                }); 
            } 
        ...
        function renderMap() { 
            ...
            activeLayout.forEach(col => { 
                if (col.aisle) { vHtml += `<div class="w-14 h-[420px] bg-yellow-50/50 flex flex-col items-center justify-center border-x-2 border-yellow-300 shadow-inner rounded-sm mx-1"><span class="text-yellow-600 font-black tracking-widest text-xs" style="writing-mode: vertical-rl;">통로</span></div>`; } 
                    ...
                    for (let r = col.cols; r >= 1; r--) { 
                        let dbId = `${col.id}-${r.toString().padStart(2, '0')}`; let displayId = `${col.id}${r}`; let searchId = floor === "1" ? dbId : `${dbId}-2F`; let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === displayId) cellState = 'cell-active'; 
            ...
            for (let c = hCols; c >= 1; c--) { 
                let displayId = `${hPrefix}${c}`; let dbId = `${hPrefix}-${c.toString().padStart(2, '0')}`; let searchId = floor === "1" ? dbId : `${dbId}-2F`; let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === displayId) cellState = 'cell-active'; 
                let pCount = palletMap[searchId] || 0; let badge = (pCount > 1) ? `<div class="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] font-black px-1 rounded-full shadow-sm z-10">${pCount.toFixed(1)}P</div>` : ''; 
        ...
        async function clickCell(displayId, searchId) { 
            selectedCellId = displayId; if(!searchId) { const floor = document.getElementById('floor-select').value; const baseId = displayId.replace(/([A-Z])([0-9]+)/, (m, p1, p2) => `${p1}-${p2.padStart(2, '0')}`); searchId = floor === "1" ? baseId : `${baseId}-2F`; } renderMap(); 
        ...
        function updateDashboard() { 
            ...
            let totalRoom = 0, occRoom = 0, totalCold = 0, occCold = 0; let valRoom = 0, valCold = 0; const occMap = {}; globalOccupancy.forEach(item => { occMap[item.location_id] = true; }); globalOccupancy.forEach(item => { let p = item.pallet_count || 1; let pInfo = productMaster.find(prod => prod.item_name === item.item_name && prod.supplier === item.remarks); let val = pInfo ? pInfo.unit_price * item.quantity : 0; if(item.location_id.startsWith('K-') || layoutRoom.some(col => col.id && item.location_id.startsWith(col.id+'-'))) { occRoom += p; valRoom += val; } else if(item.location_id.startsWith('I-') || layoutCold.some(col => col.id && item.location_id.startsWith(col.id+'-'))) { occCold += p; valCold += val; } else { valRoom += val; } }); 
