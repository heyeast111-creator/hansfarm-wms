...
<body class="font-sans h-screen flex overflow-hidden text-slate-800 selection:bg-indigo-200">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm">
                            <th class="p-4 font-black">카테고리</th><th class="p-4 font-black">품목명 (입고처)</th><th class="p-4 font-black text-right">현재 총 재고</th><th class="p-4 font-black text-right">일간 소모량</th><th class="p-4 font-black text-center">예상 소진일</th><th class="p-4 font-black text-center">상태 및 발주</th>
                        </tr>
                    </thead>
    ...
    <script>
        ...
        function adminLogin() {
            ...
            const pw = prompt("비밀번호 입력:"); if(pw === "00700") { isAdmin = true; alert("관리자 권한 활성화"); document.getElementById('admin-btn').innerText = "🔓 관리자 권한 (ON)"; document.getElementById('admin-btn').className = "w-full py-3 rounded-md bg-rose-600 border border-rose-700 text-white font-black shadow-inner text-[11px] transition-all hover:bg-rose-500 animate-pulse"; load(); }
        }
        ...
        // ==========================================
        // 📋 1. 재고 실사 양식(클립보드용) 다운로드 생성기
        // ==========================================
        function generateAllLocations() {
            let locs = [];
            // 실온 창고
            layoutRoom.forEach(col => {
                if(col.cols) { for(let r=1; r<=col.cols; r++) { locs.push({ zone: '창고-실온(1F)', id: `${col.id}-${r.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-실온(2F)', id: `${col.id}-${r.toString().padStart(2, '0')}-2F` }); } }
            });
            ...
            // 냉장 창고
            layoutCold.forEach(col => {
                if(col.cols) { for(let r=1; r<=col.cols; r++) { locs.push({ zone: '창고-냉장(1F)', id: `${col.id}-${r.toString().padStart(2, '0')}` }); locs.push({ zone: '창고-냉장(2F)', id: `${col.id}-${r.toString().padStart(2, '0')}-2F` }); } }
            });
        ...
        function exportPhysicalCountExcel() {
            if(!confirm("창고 및 현장의 모든 렉(빈 칸 포함)을 수기 실사용 엑셀로 다운로드합니다.\\n출력하여 현장에 들고 나가실 수 있습니다.")) return;
            
            const allLocs = generateAllLocations();
            const wsData = [];
            ...
            allLocs.forEach(loc => {
                ...
                if (items.length > 0) {
                    items.forEach(item => {
                        wsData.push({
                            "구역": loc.zone, "렉 위치": loc.id, "품목명": item.item_name, "입고처": item.remarks || "", "전산 수량 (박스)": item.quantity,
                            "실제 수량 (수기작성)": "              ", "차이 사유 / 비고": ""
                        });
                    });
                } else {
                    wsData.push({
                        "구역": loc.zone, "렉 위치": loc.id, "품목명": "(빈 칸)", "입고처": "", "전산 수량 (박스)": 0,
                        "실제 수량 (수기작성)": "              ", "차이 사유 / 비고": ""
                    });
                }
            });
            ...
            // 열 너비 조절 (출력용)
            ws['!cols'] = [ {wch: 15}, {wch: 15}, {wch: 25}, {wch: 15}, {wch: 15}, {wch: 25}, {wch: 30} ];
            
            XLSX.utils.book_append_sheet(wb, ws, "현장 재고실사 양식"); 
            XLSX.writeFile(wb, "한스팜_재고실사양식(수기작성용).xlsx");
        }

        // ==========================================
        // 이하 기존 통신 및 렌더링 로직 (유지)
        // ==========================================
        function renderProductMaster() { 
            document.getElementById('pm-list').innerHTML = productMaster.map(p => { 
                ...
                return `<div class="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-200">
                    ...
                    <div class="flex space-x-1">
                        <button onclick="editProductSetup('${p.category}', '${p.item_name}', '${p.supplier}', ${p.daily_usage}, ${p.unit_price})" class="text-blue-500 hover:bg-blue-100 p-1.5 rounded transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></button>
                        ${delBtn}
                    </div>
                </div>`; 
            }).join(''); 
        ...
        function editProductSetup(cat, name, supplier, usage, price) {
            editingProductOriginalName = name; editingProductOriginalSupplier = supplier;
            document.getElementById('pm-cat').value = cat; document.getElementById('pm-name').value = name; document.getElementById('pm-supplier').value = supplier; document.getElementById('pm-usage').value = usage; document.getElementById('pm-price').value = price;
        ...
        function cancelEdit() {
            ...
            document.getElementById('pm-cat').value = ''; document.getElementById('pm-name').value = ''; document.getElementById('pm-supplier').value = ''; document.getElementById('pm-usage').value = '0'; document.getElementById('pm-price').value = '0';
        ...
        async function submitProduct() {
            const cat = document.getElementById('pm-cat').value.trim(); const name = document.getElementById('pm-name').value.trim(); const supplier = document.getElementById('pm-supplier').value.trim() || '기본입고처'; const usage = parseInt(document.getElementById('pm-usage').value) || 0; const price = parseInt(document.getElementById('pm-price').value) || 0;
            if(!cat || !name) return alert("카테고리와 품목명은 필수입니다.");
            try { 
                if(editingProductOriginalName) {
                    await fetch(`/api/products?old_name=${encodeURIComponent(editingProductOriginalName)}&old_supplier=${encodeURIComponent(editingProductOriginalSupplier)}`, { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify({category: cat, item_name: name, supplier: supplier, daily_usage: usage, unit_price: price}) });
                    alert("수정 완료"); cancelEdit();
                } else {
                    await fetch('/api/products', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({category: cat, item_name: name, supplier: supplier, daily_usage: usage, unit_price: price}) });
        ...
        function generateKakaoText(itemName) { const supplier = prompt(`[${itemName}] 발주처(업체명)를 입력하세요:\\n(예: 서울계란)`); if(!supplier) return; const moq = prompt(`[${supplier}]에 요청할 수량을 입력하세요:\\n(예: 1000박스)`, "1000개"); if(!moq) return; const leadTime = prompt(`납기 요청일을 입력하세요:`, "최대한 빠르게"); const text = `[발주 요청서]\\n수신: ${supplier}\\n\\n안녕하세요, 한스팜입니다.\\n아래 품목 발주 요청드립니다.\\n\\n- 품목명: ${itemName}\\n- 발주수량: ${moq}\\n- 납기요청: ${leadTime}\\n\\n확인 후 회신 부탁드립니다. 감사합니다.`; navigator.clipboard.writeText(text.replace(/\\n/g, '\\n')).then(() => { alert("✅ 카카오톡 발주 멘트가 복사되었습니다! 채팅방에 붙여넣기(Ctrl+V) 하세요."); }); }
        ...
        function exportExcel() { let wsData = []; if(globalOccupancy.length === 0) { alert("재고가 없어 양식만 다운로드합니다."); wsData = [{ "렉 위치": "", "카테고리": "", "품목명": "", "수량": "", "단가(원)": "", "총비용(원)": "", "산란일": "", "입고처(비고)": "" }]; } else { wsData = globalOccupancy.map(item => { let pInfo = productMaster.find(p => p.item_name === item.item_name && p.supplier === item.remarks); let price = pInfo ? pInfo.unit_price : 0; return { "렉 위치": item.location_id, "카테고리": item.category, "품목명": item.item_name, "수량": item.quantity, "단가(원)": price, "총비용(원)": price * item.quantity, "산란일": item.production_date || "", "입고처(비고)": item.remarks || "" }; }); } const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(wsData), "재고 및 자산"); XLSX.writeFile(wb, "한스팜_재고자산현황.xlsx"); }
        ...
        function renderMap() { const floor = document.getElementById('floor-select').value; const vContainer = document.getElementById('vertical-racks'); const hContainer = document.getElementById('horizontal-rack'); const occMap = {}; globalOccupancy.forEach(item => { occMap[item.location_id] = true; }); let vHtml = ''; hContainer.innerHTML = ''; if(currentZone === '현장') { document.getElementById('aisle-text').classList.add('hidden'); vHtml += `<div class="w-full min-w-[700px]">`; layoutFloor.forEach(col => { if(col.aisle) { vHtml += `<div class="w-full h-10 bg-yellow-50/50 flex items-center justify-center border-y-2 border-yellow-300 shadow-inner my-6 rounded-lg"><span class="text-yellow-600 font-black tracking-widest text-sm">${col.text}</span></div>`; } else { let colorClass = col.id === 'FL-C' ? 'text-indigo-800 bg-indigo-50 border-indigo-200' : 'text-orange-800 bg-orange-50 border-orange-200'; vHtml += `<div class="mb-4 bg-white p-5 rounded-2xl shadow-md border border-slate-200"><div class="text-lg font-black ${colorClass} p-3 rounded-lg border mb-4 shadow-sm inline-block">${col.title}</div><div class="grid grid-cols-10 gap-3">`; for (let r = 1; r <= col.cols; r++) { let dbId = `${col.id}-${r.toString().padStart(2, '0')}`; let searchId = dbId; let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === dbId) cellState = 'cell-active'; vHtml += `<div id="cell-${dbId}" onclick="clickCell('${dbId}', '${searchId}')" class="h-16 rounded-xl border-2 flex flex-col items-center justify-center text-[11px] font-black cursor-pointer rack-cell ${cellState} shadow-sm hover:scale-105 transition-all"><span class="${hasItem?'text-slate-700':'text-slate-400'}">${r}번 칸</span></div>`; } vHtml += `</div></div>`; } }); vHtml += `</div>`; vContainer.innerHTML = vHtml; return; } document.getElementById('aisle-text').classList.remove('hidden'); document.getElementById('aisle-text').innerText = "통로 (Aisle)"; const activeLayout = currentZone === '실온' ? layoutRoom : layoutCold; activeLayout.forEach(col => { if (col.aisle) { vHtml += `<div class="w-14 h-[420px] bg-yellow-50/50 flex flex-col items-center justify-center border-x-2 border-yellow-300 shadow-inner rounded-sm mx-1"><span class="text-yellow-600 font-black tracking-widest text-xs" style="writing-mode: vertical-rl;">통로</span></div>`; } else if (col.gap) { vHtml += `<div class="w-4"></div>`; } else { vHtml += `<div class="flex flex-col w-14 space-y-1 justify-end"><div class="text-center font-black text-2xl text-slate-800 pb-2">${col.id}</div>`; for (let r = col.cols; r >= 1; r--) { let dbId = `${col.id}-${r.toString().padStart(2, '0')}`; let displayId = `${col.id}${r}`; let searchId = floor === "1" ? dbId : `${dbId}-2F`; let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === displayId) cellState = 'cell-active'; vHtml += `<div id="cell-${displayId}" onclick="clickCell('${displayId}', '${searchId}')" class="h-8 rounded-[3px] flex items-center justify-center text-[10px] font-bold cursor-pointer rack-cell ${cellState} shadow-sm">${displayId}</div>`; } vHtml += `</div>`; } }); vContainer.innerHTML = vHtml; let hHtml = `<div class="flex space-x-1">`; let hPrefix = currentZone === '실온' ? 'K' : 'I'; let hCols = currentZone === '실온' ? 10 : 8; for (let c = hCols; c >= 1; c--) { let displayId = `${hPrefix}${c}`; let dbId = `${hPrefix}-${c.toString().padStart(2, '0')}`; let searchId = floor === "1" ? dbId : `${dbId}-2F`; let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === displayId) cellState = 'cell-active'; hHtml += `<div id="cell-${displayId}" onclick="clickCell('${displayId}', '${searchId}')" class="w-14 h-10 rounded-[3px] flex items-center justify-center text-[11px] font-bold cursor-pointer rack-cell ${cellState} shadow-sm">${displayId}</div>`; } hHtml += `</div>`; hContainer.innerHTML = hHtml; }
            ...
            panelHtml += `
                <div class="mt-4 pt-4 border-t border-slate-200">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex space-x-2"><div class="w-1/2"><label class="block text-[10px] font-bold text-rose-500 mb-1">입고처 (단가 매칭용)</label><select id="in-supplier" class="w-full border border-rose-300 bg-rose-50 rounded p-1.5 text-xs font-bold"><option value="">품목 선택</option></select></div><div class="w-1/2"><label class="block text-[10px] font-bold text-slate-400 mb-1">수량</label><input type="number" id="in-qty" value="1" min="1" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"></div></div>
                        <div><label class="block text-[10px] font-bold text-slate-400 mb-1">산란일</label><input type="date" id="in-date" class="w-full border border-slate-300 rounded p-1.5 text-xs font-bold"></div>
        ...
        async function processInbound(locId) { const cat = document.getElementById('in-cat').value; const item = document.getElementById('in-item').value; const qty = document.getElementById('in-qty').value; const date = document.getElementById('in-date').value; const supplier = document.getElementById('in-supplier').value; if(!cat || !item || !qty) return alert("필수값 입력 요망"); try { await fetch('/api/inbound', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ location_id: locId, category: cat, item_name: item, quantity: parseInt(qty), production_date: date || null, remarks: supplier }) }); alert("입고 완료!"); load(); } catch(e) {} }
