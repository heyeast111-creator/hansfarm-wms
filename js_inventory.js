...
// ==========================================
// 🚨 [보안 터널 완벽 복구] 재고 실사 전용 동기화 (덮어쓰기/수정/삭제/유지)
// ==========================================
async function importExcel(event) {
    ...
    try {
        ...
        for (let row of jsonData) {
            ...
            // 💡 [핵심 조건 2] 엑셀의 '전산수량'과 '실사수량'이 똑같으면 서버 부하 방지를 위해 쿨하게 패스!
            ...
            // 만약 품목명이 없고 수량이 0이면 해당 렉의 전체를 비우려는 의도인지 체크
            if(itemName === "") existings = globalOccupancy.filter(o => o.location_id === loc);

            // 💡 [핵심 조건 3] 실사수량이 0이면 해당 데이터 삭제
            if (newQty === 0) {
                if(existings.length > 0) {
                    existings.forEach(e => {
                        deletes.push({ invId: e.id, locId: loc, itemName: e.item_name });
                    });
                    processCount++;
                }
            } 
            // 💡 [핵심 조건 4] 실사수량이 1 이상이면 덮어쓰기(UPDATE) 또는 신규생성(INSERT)
            else {
                ...
                if (existings.length === 0) {
                    ...
                    inserts.push({
                        ...
                        production_date: pDate || new Date().toISOString().split('T')[0], // 실사일 기준
                        remarks: remarks || "기본입고처"
                    });
                    processCount++;
                } else {
                    // 렉에 물건이 있었으면, 엑셀에 적힌 이름/수량으로 무조건 덮어쓰기!
                    updates.push({
                    ...
                    // 혹시나 전산 오류로 같은 위치에 같은 물건이 여러 줄 쪼개져 있다면 하나로 합치기 위해 나머지는 삭제
                    for(let i=1; i<existings.length; i++) {
                        deletes.push({ invId: existings[i].id, locId: loc, itemName: existings[i].item_name });
                    }
                    processCount++;
                }
            }
        ...
        if (!confirm(`✅ 총 ${processCount}건의 렉(위치)에서 재고 변동이 감지되었습니다.\n\n이대로 장부를 완전히 동기화(덮어쓰기) 하시겠습니까?\n(※ 파이썬 정식 서버를 통해 100% 안전하게 처리됩니다.)`)) {
            event.target.value = '';
            return;
        }

        // 💡 [핵심 패치] 외부 링크(Supabase) 직통 터널 폭파! -> 브라우저 보안 에러 원천 차단
        // 파이썬 서버의 정식 API(/api/inventory_edit, /api/inbound) 만을 사용하여 안전하게 명령 하달!
        ...
        for(let u of updates) {
            await fetch('/api/inventory_edit', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ inventory_id: u.invId, location_id: u.locId, item_name: u.itemName, action: 'UPDATE_QTY', new_quantity: u.new_quantity, pallet_count: u.pallet_count }) });
        }
        ...
        alert("🎉 재고 실사 결과가 서버에 완벽하게 동기화되었습니다!");
        event.target.value = ''; 
