...
function toggleMapSearch() { const c = document.getElementById('map-search-container'); if(c.classList.contains('hidden')) { c.classList.remove('hidden'); c.classList.add('flex'); } else { c.classList.add('hidden'); c.classList.remove('flex'); } }
function toggleWaitContainer() { const c = document.getElementById('wait-container'); if(c.classList.contains('hidden')) { c.classList.remove('hidden'); c.classList.add('flex'); } else { c.classList.add('hidden'); c.classList.remove('flex'); } }
function toggleWaitManualInput() { let sel = document.getElementById('wait-item'); let man = document.getElementById('wait-item-manual'); if (sel.value === 'DIRECT_INPUT') { sel.classList.add('hidden'); man.classList.remove('hidden'); man.focus(); } else { sel.classList.remove('hidden'); man.classList.add('hidden'); } }

function getDynamicPalletCount(itemObj) {
    if(!itemObj) return 0;
    let itemName = String(itemObj.item_name || "").trim(); let supplier = String(itemObj.remarks || "기본입고처").trim().replace(/\[기존재고\]/g, '').trim();
...
function changeFloorCols(areaId, delta) {
    ...
    if(delta < 0) { let targetId = `${areaId}-${currentCols.toString().padStart(2, '0')}`; if(globalOccupancy.some(item => item.location_id === targetId)) return alert(`재고가 남아있어 줄일 수 없습니다.`); }
...
function renderMap() { 
    try {
        ...
        globalOccupancy.forEach(item => { occMap[item.location_id] = true; palletMap[item.location_id] = (palletMap[item.location_id] || 0) + getDynamicPalletCount(item); }); 
        ...
        if(currentZone === '현장') { 
            ...
            let oldGhostItems = globalOccupancy.filter(o => { if (!String(o.location_id).startsWith('FL-')) return false; return !newPatterns.some(pat => String(o.location_id).startsWith(pat)); });
            
            if (oldGhostItems.length > 0) {
                let oldIds = [...new Set(oldGhostItems.map(o => o.location_id))].sort();
                vHtml += `<div class="mb-8 bg-rose-100 p-6 rounded-2xl shadow-xl border-4 border-rose-500 w-full max-w-5xl mx-auto relative z-10"><div class="text-lg md:text-xl font-black text-rose-800 mb-2 flex items-center animate-pulse">🚨 경고: [이전 맵 구형 재고] 감지됨!</div><div class="text-sm font-bold text-rose-600 mb-4 bg-white p-2 rounded inline-block shadow-sm">우측 패널에서 편집/삭제 -> 3번으로 비워주세요.</div><div class="flex flex-wrap gap-3">`;
                oldIds.forEach(searchId => {
                    let pCount = palletMap[searchId] || 0; let badge = (pCount > 1) ? `<div class="absolute -top-2 -right-2 bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-md z-10">${pCount.toFixed(1)}P</div>` : ''; 
            ...
            const prodSiteConfig = [{ id: 'FL-1F', name: '생산현장 1층', areas: [{ key: 'R', title: '🥚 원란창고', color: 'orange', bgColor: 'bg-orange-50', borderColor: 'border-orange-200', textColor: 'text-orange-800' }, { key: 'M', title: '📦 자재창고', color: 'blue', bgColor: 'bg-blue-50', borderColor: 'border-blue-200', textColor: 'text-blue-800' }, { key: 'P', title: '✨ 제품창고', color: 'emerald', bgColor: 'bg-emerald-50', borderColor: 'border-emerald-200', textColor: 'text-emerald-800' }]}, { id: 'FL-2F', name: '생산현장 2층', areas: [{ key: 'M', title: '📦 자재창고', color: 'blue', bgColor: 'bg-blue-50', borderColor: 'border-blue-200', textColor: 'text-blue-800' }, { key: 'P', title: '✨ 제품창고', color: 'emerald', bgColor: 'bg-emerald-50', borderColor: 'border-emerald-200', textColor: 'text-emerald-800' }]}, { id: 'FL-3F', name: '생산현장 3층', areas: [{ key: 'G', title: '🏭 일반창고(공용)', color: 'slate', bgColor: 'bg-slate-50', borderColor: 'border-slate-200', textColor: 'text-slate-800' }]}];
            ...
            prodSiteConfig.forEach(floorInfo => { 
                ...
                floorInfo.areas.forEach(area => {
                    ...
                    for (let r = 1; r <= cols; r++) { 
                        let dbId = `${areaId}-${r.toString().padStart(2, '0')}`; let hasItem = occMap[dbId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === dbId) cellState = 'cell-active'; 
                        let pCount = palletMap[dbId] || 0; let badge = (pCount > 1) ? `<div class="absolute -top-1 -right-1 bg-rose-500 text-white text-[8px] font-black px-1 rounded-full z-10 scale-90">${pCount.toFixed(1)}P</div>` : ''; 
                        let isTarget = globalSearchTargets.includes(dbId); let pulseClass = isTarget ? 'highlight-pulse' : ''; if(movingItem && movingItem.fromLoc === dbId) pulseClass += ' highlight-move';
            ...
            vHtml += `</div>`; if(vContainer) vContainer.innerHTML = vHtml; return; 
        } 
        ...
        activeLayout.forEach(col => { 
            if (col.aisle) { vHtml += `<div class="w-10 md:w-14 h-[420px] bg-yellow-50/50 flex flex-col items-center justify-center border-x-2 border-yellow-300 shadow-inner rounded-sm mx-1"><span class="text-yellow-600 font-black tracking-widest text-[10px]" style="writing-mode: vertical-rl;">통로</span></div>`; } 
                ...
                for (let r = col.cols; r >= 1; r--) { 
                    ...
                    let hasItem = occMap[searchId]; let cellState = hasItem ? 'cell-full' : 'cell-empty'; if(selectedCellId === displayId || selectedCellId === searchId) cellState = 'cell-active'; 
                    ...
                    let isTarget = globalSearchTargets.includes(searchId); let pulseClass = isTarget ? 'highlight-pulse' : ''; if(movingItem && movingItem.fromLoc === searchId) pulseClass += ' highlight-move';
...
async function clickCell(displayId, searchId) { 
    try {
        ...
        let panelHtml = `<div class="bg-indigo-50 p-4 rounded-lg border border-indigo-200 mb-4 flex justify-between items-center"><div><div class="text-[10px] text-indigo-500 font-bold mb-1">선택 위치</div><div class="text-2xl font-black text-indigo-900">${displayId}</div></div><button onclick="showHistoryModal('${searchId}')" class="text-[10px] bg-white border border-indigo-200 px-2 py-1 rounded font-bold text-indigo-600 shadow-sm">기록보기</button></div>`; 
        ...
        if (!searchId.startsWith('W-')) {
            ...
            if(locHistory.length > 0) { locHistory.forEach(h => { let actionColor = h.action_type === '입고' ? 'text-emerald-600' : 'text-rose-600'; panelHtml += `<div class="bg-white p-2 border border-slate-200 rounded text-[10px] shadow-sm"><span class="font-bold ${actionColor}">[${h.action_type}]</span> <span class="text-slate-500">${h.production_date || new Date(h.created_at).toISOString().split('T')[0]}</span><br><span class="font-bold text-slate-700">${h.item_name} (${h.quantity}EA)</span></div>`; }); } else { panelHtml += `<div class="text-[10px] text-slate-400 text-center py-4">기록 없음</div>`; }
            panelHtml += `</div></div>`;
        }
...
async function dispatchToFloor(invId, itemName, maxQty, fromLoc, supplier) {
    ...
    } else if (floorStr === "2") { targetPrefix = isFinished ? 'FL-2F-P-' : 'FL-2F-M-'; } 
    else if (floorStr === "3") { targetPrefix = 'FL-3F-G-'; }
...
async function processOutbound(invId, itemName, maxQty, currentPallet, locId) { 
    ...
    if(isNaN(qty) || qty<=0 || qty>maxQty) return alert("오류");
...
// 💡 대기장 & 기타 직접입력
// ==========================================
function getWaitZoneSourceItems() { return currentZone === '실온' ? productMaster.filter(p => !p.category?.includes('원란')) : (currentZone === '냉장' ? productMaster.filter(p => p.category?.includes('원란')) : [...finishedProductMaster, ...productMaster]); }
...
async function createWaitingPallets() {
    ...
    if(payloads.length > 0) { try { await Promise.all(payloads.map(p => fetch('/api/inbound', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(p) }))); document.getElementById('wait-qty').value = ''; document.getElementById('wait-pal').value = ''; document.getElementById('wait-item-manual').value = ''; toggleWaitManualInput(); await load(); } catch(e){} }
}
...
function renderDailyInventory() {
    let targetDate = document.getElementById('daily-target-date').value; if(!targetDate) return;
    let dailyStock = {};
    globalHistory.forEach(h => {
        let hDate = new Date(h.created_at).toISOString().split('T')[0];
        if (hDate <= targetDate) {
            let itemName = String(h.item_name || "미상").trim(); let cat = String(h.category || "미분류").trim(); let qty = parseInt(h.quantity) || 0;
    ...
    let tbody = document.getElementById('daily-inventory-list'); if(!tbody) return;
    if(stockList.length === 0) { tbody.innerHTML = `<tr><td colspan="3" class="p-10 text-center text-slate-400 font-bold">기록이 없습니다.</td></tr>`; return; }
    tbody.innerHTML = stockList.map(item => `<tr class="hover:bg-indigo-50/50"><td class="p-3 text-xs text-slate-500 font-bold">${item.category}</td><td class="p-3 text-sm font-black text-slate-800">${item.item_name}</td><td class="p-3 text-right text-sm font-black text-indigo-600">${item.qty.toLocaleString()} <span class="text-xs text-indigo-400">EA</span></td></tr>`).join('');
}

function exportDailyInventoryExcel() {
    let targetDate = document.getElementById('daily-target-date').value; let tbody = document.getElementById('daily-inventory-list');
    if(!tbody || tbody.innerText.includes('기록이 없습니다')) return alert("다운로드할 데이터가 없습니다.");
    let wsData = [];
    tbody.querySelectorAll('tr').forEach(tr => { let cols = tr.querySelectorAll('td'); if(cols.length === 3) wsData.push({ "기준일자": targetDate, "카테고리": cols[0].innerText.trim(), "품목명": cols[1].innerText.trim(), "총 수량(EA)": parseInt(cols[2].innerText.replace(/[^0-9-]/g, '')) || 0 }); });
    const wb = XLSX.utils.book_new(); const ws = XLSX.utils.json_to_sheet(wsData); XLSX.utils.book_append_sheet(wb, ws, `재고_${targetDate}`); XLSX.writeFile(wb, `한스팜_재고_${targetDate}.xlsx`);
}
...
function updateMapSearchItemDropdown() {
    let sourceItems = currentZone === '실온' ? productMaster.filter(p => p.category && !p.category.includes('원란')) : (currentZone === '냉장' ? productMaster.filter(p => p.category && p.category.includes('원란')) : finishedProductMaster);
    const catSelect = document.getElementById('map-search-category'); if (catSelect && catSelect.value !== 'ALL') sourceItems = sourceItems.filter(p => p.category === catSelect.value); 
    const uniqueItems = [...new Set(sourceItems.map(p => p.item_name))].filter(Boolean).sort();
    const datalist = document.getElementById('map-search-item-list'); if(datalist) datalist.innerHTML = uniqueItems.map(name => `<option value="${name}">`).join('');
}
...
function highlightFIFO() { 
    const eggs = globalOccupancy.filter(x => x.production_date && x.location_id.startsWith('C-')); if(eggs.length === 0) return alert("냉장 산란일 없음"); 
    eggs.sort((a, b) => new Date(a.production_date) - new Date(b.production_date)); const oldestDate = eggs[0].production_date; 
    globalSearchTargets = eggs.filter(x => x.production_date === oldestDate).map(t => t.location_id);
    currentZone = '냉장'; document.getElementById('floor-select').value = globalSearchTargets[0].endsWith('-2F') ? "2" : "1";
...
function updateSummaryCategoryDropdown() {
    let items = getSummarySourceItems(); const sup = document.getElementById('summary-supplier')?.value || 'ALL'; if (sup !== 'ALL') items = items.filter(p => p.supplier === sup);
    ...
    if(itemName) {
        globalOccupancy.forEach(item => {
            ...
            if(item.item_name === itemName && (supplier === 'ALL' || cleanItemSup === supplier)) {
                ...
                let dynP = getDynamicPalletCount(item); breakdown[cleanItemSup].qty += item.quantity; breakdown[cleanItemSup].pallet += dynP; totalQty += item.quantity; totalPallet += dynP;
            }
        });
    }
    document.getElementById('summary-result').innerHTML = `${totalQty.toLocaleString()} <span class="text-xl text-indigo-400">EA</span>`; document.getElementById('summary-pallet').innerText = `${totalPallet.toFixed(1)} P`;
    let breakdownHtml = '<div class="space-y-2 mt-4">'; Object.keys(breakdown).forEach(sup => { breakdownHtml += `<div class="flex justify-between items-center bg-white p-3 rounded border border-slate-200"><span class="font-bold w-1/3 truncate">${sup}</span><span class="font-black text-indigo-600 w-1/3 text-right">${breakdown[sup].qty.toLocaleString()} EA</span><span class="font-bold text-rose-500 w-1/3 text-right">${breakdown[sup].pallet.toFixed(1)} P</span></div>`; }); document.getElementById('summary-breakdown').innerHTML = Object.keys(breakdown).length > 0 ? breakdownHtml + '</div>' : '';
}
function findItemLocationFromSummary() {
    const itemName = document.getElementById('summary-item').value; const supplier = document.getElementById('summary-supplier').value;
    if(!itemName) return alert("품목을 선택하세요.");
    let targets = globalOccupancy.filter(item => { let cleanSup = String(item.remarks || "기본입고처").replace(/\[기존재고\]/g, '').trim(); return item.item_name === itemName && (supplier === 'ALL' || cleanSup === supplier); });
    if(targets.length === 0) return alert("재고가 없습니다.");
    globalSearchTargets = targets.map(t => t.location_id); let firstLoc = globalSearchTargets[0];
    if (firstLoc.startsWith('FL-')) currentZone = '현장'; else if (firstLoc.startsWith('C-')) { currentZone = '냉장'; document.getElementById('floor-select').value = firstLoc.endsWith('-2F') ? "2" : "1"; } else { currentZone = '실온'; document.getElementById('floor-select').value = firstLoc.endsWith('-2F') ? "2" : "1"; }
...
// 💡 발주 장바구니 & 내역
// ==========================================
function toggleOrderCart() { const el = document.getElementById('order-cart-container'); el.classList.toggle('hidden'); el.classList.toggle('flex'); updateOrderCartDropdowns(); }
function updateOrderCartDropdowns() { let sups = [...new Set(productMaster.map(p=>p.supplier))].filter(Boolean).sort(); let supSel = document.getElementById('oc-sup'); if(supSel) { let cur = supSel.value; supSel.innerHTML = '<option value="">1. 발주처 선택</option>' + sups.map(s=>`<option value="${s}">${s}</option>`).join(''); if(sups.includes(cur)) supSel.value = cur; updateOrderCartCategoryDropdown(); } }
function updateOrderCartCategoryDropdown() { let sup = document.getElementById('oc-sup').value; let cats = [...new Set(productMaster.filter(p=>p.supplier===sup).map(p=>p.category))].filter(Boolean).sort(); let catSel = document.getElementById('oc-cat'); if(catSel) { let cur = catSel.value; catSel.innerHTML = '<option value="">2. 카테고리</option>' + cats.map(c=>`<option value="${c}">${c}</option>`).join(''); if(cats.includes(cur)) catSel.value = cur; updateOrderCartItemDropdown(); } }
function updateOrderCartItemDropdown() { let sup = document.getElementById('oc-sup').value; let cat = document.getElementById('oc-cat').value; let items = [...new Set(productMaster.filter(p=>p.supplier===sup && p.category===cat).map(p=>p.item_name))].filter(Boolean).sort(); let itemSel = document.getElementById('oc-item'); if(itemSel) itemSel.innerHTML = '<option value="">3. 품목 선택</option>' + items.map(c=>`<option value="${c}">${c}</option>`).join(''); }
function addOrderCartItem() {
    let cat = document.getElementById('oc-cat').value; let item = document.getElementById('oc-item').value; let sup = document.getElementById('oc-sup').value; let pal = parseFloat(document.getElementById('oc-pal').value); let expDate = document.getElementById('oc-date') ? document.getElementById('oc-date').value : new Date().toISOString().split('T')[0];
    ...
    orderCart.push({ category: cat, item_name: item, supplier: sup, pallet_count: pal, quantity: totalQty, expected_date: expDate }); document.getElementById('oc-pal').value = ''; renderOrderCart();
}
...
async function submitOrderCart() {
    ...
    orderCart.forEach((item, i) => { 
        ...
        dbPayloads.push({ location_id: "[발주대기]", action_type: "발주중", category: item.category || '미분류', item_name: item.item_name, remarks: item.supplier, quantity: item.quantity, pallet_count: item.pallet_count, production_date: item.expected_date, payment_status: "미지급", acc_status: "미확정" });
    });
    try { 
        ...
        try { await navigator.clipboard.writeText(text); alert("발주 등록 및 카카오톡 텍스트 복사 완료!"); } catch(err) { alert("발주 등록 완료! (직접 복사해주세요)"); }
...
function initOrderSortUI() {
    try {
        const container = document.getElementById('subview-history'); if(!container) return; if(document.getElementById('order-sort-select')) return;
        ...
        const tableEl = container.querySelector('table'); if(tableEl && tableEl.parentElement) tableEl.parentElement.insertAdjacentHTML('beforebegin', sortHtml); else container.insertAdjacentHTML('afterbegin', sortHtml);
    } catch(e) {}
}
...
async function receiveOrder(logId, itemName, qty, pallet, supplier, cat) {
    if(loginMode === 'viewer') return; 
    let receiveDate = prompt(`[${itemName}] 실제 도착(입고)일 (YYYY-MM-DD):`, new Date().toISOString().split('T')[0]); if (receiveDate === null) return;
    ...
    if (!confirm(`📦 현재 마스터 1P 기준: ${defaultPEa.toLocaleString()} EA\n\n이 기준대로 대기장에 자동 분할할까요?\n[취소]를 누르면 직접 입력합니다.`)) {
        let manualEa = prompt(`1박스당 낱개 수량(EA)을 입력하세요:`, defaultPEa); if(manualEa === null) return; pEa = parseInt(manualEa);
    }
    let remaining = qty; let payloads = []; let waitIndex = 1;
    while(remaining > 0) { 
        ...
        if(!emptyW) { if(payloads.length === 0) return alert(`대기장이 꽉 찼습니다!`); else { alert(`일부만 입고 처리됩니다.`); break; } } 
        payloads.push({ location_id: emptyW, category: cat || '미분류', item_name: itemName, quantity: chunk, pallet_count: chunk / pEa, production_date: receiveDate, remarks: supplier }); remaining -= chunk; 
    }
    try { 
...
function openEditOrderModal(logId) {
    if(loginMode === 'viewer') return;
    let order = globalHistory.find(h => h.id === logId); if(!order) return;
    document.getElementById('edit-order-id').value = logId;
    let sup = order.remarks || '기본입고처'; document.getElementById('edit-order-supplier').value = sup; document.getElementById('edit-order-supplier-text').innerText = sup;
    let cats = [...new Set(productMaster.filter(p=>p.supplier===sup).map(p=>p.category))].filter(Boolean).sort();
    let catSel = document.getElementById('edit-order-cat'); catSel.innerHTML = cats.map(c=>`<option value="${c}">${c}</option>`).join('');
    if(cats.includes(order.category)) catSel.value = order.category;
    updateEditOrderCategoryDropdown(order.item_name);
    document.getElementById('edit-order-qty').value = order.quantity; document.getElementById('edit-order-date').value = order.production_date;
    document.getElementById('edit-order-modal').classList.remove('hidden'); document.getElementById('edit-order-modal').classList.add('flex');
}
function updateEditOrderCategoryDropdown(selectedItem = null) {
    let sup = document.getElementById('edit-order-supplier').value; let cat = document.getElementById('edit-order-cat').value;
    let items = [...new Set(productMaster.filter(p=>p.supplier===sup && p.category===cat).map(p=>p.item_name))].filter(Boolean).sort();
    let itemSel = document.getElementById('edit-order-item'); itemSel.innerHTML = items.map(i=>`<option value="${i}">${i}</option>`).join('');
    if(selectedItem && items.includes(selectedItem)) itemSel.value = selectedItem;
}
function closeEditOrderModal() { document.getElementById('edit-order-modal').classList.add('hidden'); document.getElementById('edit-order-modal').classList.remove('flex'); }
async function submitEditOrder() {
    let logId = document.getElementById('edit-order-id').value; let sup = document.getElementById('edit-order-supplier').value; let cat = document.getElementById('edit-order-cat').value; let newItem = document.getElementById('edit-order-item').value; let newQty = parseInt(document.getElementById('edit-order-qty').value); let newDate = document.getElementById('edit-order-date').value;
    ...
    try {
        await fetch(`/api/history/${logId}`, { method: 'DELETE' }); 
        let newPayload = [{ location_id: "[발주대기]", action_type: "발주중", category: cat || '미분류', item_name: newItem, remarks: sup, pallet_count: newQty / (pInfo && pInfo.pallet_ea > 0 ? pInfo.pallet_ea : 1), quantity: newQty, production_date: newDate, payment_status: "미지급", acc_status: "미확정" }];
        await fetch(`https://sxdldhjmatzzyfufavrm.supabase.co/rest/v1/history_log`, { method: 'POST', headers: { "apikey": "sb_publishable_gIXjo5pyqbDO55wgJq1Yxg_RbCEYEYu", "Authorization": "Bearer sb_publishable_gIXjo5pyqbDO55wgJq1Yxg_RbCEYEYu", "Content-Type": "application/json", "Prefer": "return=representation" }, body: JSON.stringify(newPayload) });
        alert("수정 완료"); closeEditOrderModal(); await load();
    } catch(e) {}
}
async function cancelOrder(logId) { if(loginMode === 'viewer') return; if(!confirm("취소하시겠습니까?")) return; await fetch(`/api/history/${logId}`, { method: 'DELETE' }); await load(); }
...
function renderSafetyStock() { 
    ...
    if (mode === 'pallet') {
        thead.innerHTML = `<tr><th class="p-3 font-black">카테고리</th><th class="p-3 font-black">품목명</th><th class="p-3 font-black text-right">재고 (EA)</th><th class="p-3 font-black text-right">파레트 (P)</th><th class="p-3 font-black text-center">발주</th></tr>`;
        ...
        monitoredList.forEach(p => { html += `<tr class="bg-white border-b"><td class="p-3 text-xs">${p.category}</td><td class="p-3 font-black text-xs">${p.item_name}</td><td class="p-3 text-right font-bold text-indigo-600">${p.total_qty.toLocaleString()}</td><td class="p-3 text-right font-black text-rose-600">${p.total_pallet.toFixed(1)}P</td><td class="p-3 text-center"><button onclick="generateKakaoText('${p.item_name}')" class="bg-yellow-400 text-[10px] px-2 py-1 rounded font-black">카톡 복사</button></td></tr>`; });
    } else {
        thead.innerHTML = `<tr><th class="p-3 font-black">카테고리</th><th class="p-3 font-black">품목명</th><th class="p-3 font-black text-right">재고</th><th class="p-3 font-black text-right">일간소모량</th><th class="p-3 font-black text-center">버팀일수</th></tr>`;
