...
async function importExcel(event) {
    ...
    try {
        ...
        for (let row of jsonData) {
            ...
            let existings = globalOccupancy.filter(o => o.location_id === loc && o.item_name === itemName);
            if(itemName === "") existings = globalOccupancy.filter(o => o.location_id === loc);

            // 실사수량이 0이면 삭제
            if (newQty === 0) {
                if(existings.length > 0) {
                    existings.forEach(e => deletes.push({ invId: e.id, locId: loc, itemName: e.item_name }));
                    processCount++;
                }
            } 
            else {
                ...
                // 수정(덮어쓰기)
                    ...
                    updates.push({
                        invId: existings[0].id,
                        locId: loc,
                        itemName: itemName,
                        new_quantity: newQty,
                        pallet_count: finalPallet
                    });
                    for(let i=1; i<existings.length; i++) deletes.push({ invId: existings[i].id, locId: loc, itemName: existings[i].item_name });
                    processCount++;
                }
            }
        }
        ...
        if (processCount === 0) {
            alert("반영할 실사 변동 내역이 없습니다.\n(실사수량이 빈칸이거나, 전산수량과 100% 동일합니다.)");
            event.target.value = '';
            return;
        }

        if (!confirm(`✅ 총 ${processCount}건의 렉(위치)에서 재고 변동이 감지되었습니다.\n\n이대로 렉맵 장부를 완전히 덮어쓰기 하시겠습니까?`)) {
            event.target.value = '';
            return;
        }

        // 💡 [핵심 패치] 메모리(globalOccupancy)를 조작해서 눈앞의 렉맵 화면부터 즉시 0초 만에 갈아엎습니다!
        let backupOccupancy = JSON.parse(JSON.stringify(globalOccupancy));

        try {
            // 1. 메모리에서 삭제 즉시 반영
            deletes.forEach(d => {
                globalOccupancy = globalOccupancy.filter(o => o.id !== d.invId);
            });

            // 2. 메모리에서 수정 즉시 반영
            updates.forEach(u => {
                let item = globalOccupancy.find(o => o.id === u.invId);
                if(item) {
                    item.quantity = u.new_quantity;
                    item.pallet_count = u.pallet_count;
                }
            });

            // 3. 메모리에 신규 입고 즉시 반영
            inserts.forEach(ins => {
                globalOccupancy.push({
                    id: ins.id,
                    location_id: ins.location_id,
                    category: ins.category,
                    item_name: ins.item_name,
                    quantity: ins.quantity,
                    pallet_count: ins.pallet_count,
                    production_date: ins.production_date,
                    remarks: ins.remarks
                });
            });

            // 💡 렉맵 강제 새로고침 (서버 무관, 무조건 화면부터 바뀜)
            renderMap();
            if(typeof updateDashboard === 'function') updateDashboard();
            
            alert("🎉 렉맵에 실사 결과가 즉시 반영되었습니다!\n(현재 서버에 안전하게 데이터 동기화를 진행 중입니다.)");
            event.target.value = ''; 

            // 💡 서버 통신은 백그라운드에서 조용히 알아서 진행 (파이썬 정식 API 사용)
            (async () => {
                try {
                    for(let d of deletes) await fetch('/api/inventory_edit', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ inventory_id: d.invId, location_id: d.locId, item_name: d.itemName, action: 'DELETE' }) });
                    for(let u of updates) await fetch('/api/inventory_edit', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ inventory_id: u.invId, location_id: u.locId, item_name: u.itemName, action: 'UPDATE_QTY', new_quantity: u.new_quantity, pallet_count: u.pallet_count }) });
                    for(let ins of inserts) await fetch('/api/inbound', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(ins) });
                    
                    // 백그라운드 저장 완료 후 진짜 서버 데이터로 한번 더 조용히 동기화
                    load();
                } catch(err) {
                    console.error("서버 백그라운드 동기화 에러:", err);
                }
            })();

        } catch(e) {
            // 자바스크립트 에러 시 기존 렉맵 상태로 롤백
            globalOccupancy = backupOccupancy;
            renderMap();
            alert("실사 렉맵 렌더링 중 오류가 발생했습니다: " + e.message);
            event.target.value = '';
        }
