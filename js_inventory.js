...
// ==========================================
// 🚨 [완전 해결] 재고 실사 무조건 덮어쓰기 (기존 데이터 삭제 후 갱신)
// ==========================================
async function importExcel(event) {
    ...
    try {
        ...
        let deletePayloads = []; // 지울 목록
        let insertPayloads = []; // 새로 넣을 목록
        let processCount = 0;
        ...
        for (let row of jsonData) {
            ...
            // 3. 덮어쓰기 대상 확정 (기존에 해당 위치에 있던 모든 데이터를 삭제 목록에 추가)
            ...
            // 4. 새로운 데이터 준비 (실사수량이 0보다 크면 삽입 목록에 추가)
            if (newQty > 0) {
        ...
        if (processCount === 0) {
            alert("반영할 실사 변동 내역이 없습니다. (실사수량 칸이 모두 비어있습니다.)");
            event.target.value = '';
            return;
        }

        if (!confirm(`⚠️ 경고: 실사수량이 입력된 ${processCount}개의 렉을 엑셀 내용으로 완전히 '덮어쓰기' 합니다.\n(입력된 렉의 기존 데이터는 삭제됩니다.)\n\n진행하시겠습니까?`)) {
            event.target.value = '';
            return;
        }

        // 💡 [실시간 UI 갱신] 메모리부터 싹 갈아엎음
        let backupOccupancy = JSON.parse(JSON.stringify(globalOccupancy));
        
        // 1. 해당 렉들의 기존 데이터 메모리에서 제거
        let locsToClear = [...new Set(deletePayloads.map(d => d.locId))];
        if (insertPayloads.length > 0) {
            // 새로 입력할 렉들도 지우기 목록에 포함시켜야 함
        ...
        alert("🎉 실사 결과가 렉맵에 즉시 반영되었습니다!\n(서버 데이터 동기화는 백그라운드에서 진행됩니다.)");
        event.target.value = '';

        // 4. 백그라운드 서버 저장 (정식 API 사용)
        (async () => {
            try {
                // 렉 비우기 (삭제)
                ...
                load(); // 최종 동기화
            } catch(err) { console.error("서버 저장 중 오류:", err); }
        })();
